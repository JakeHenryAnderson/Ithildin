...
- [docs/codex/local-v1-lv1-003-o4-attempt008-port-release-authorization.md](docs/codex/local-v1-lv1-003-o4-attempt008-port-release-authorization.md) - unconsumed exact one-shot authority for stopping only the known Attempt 008 API/UI containers, with a descriptor-anchored private sealed Docker copy, conservative invalid-lane consumption state, closed journal/disposition crosslinks, race-resistant receipt reads, and honest observed-unchanged versus not-targeted preservation semantics.
...
- `make local-v1-lv1-003-o4-attempt008-port-release-run` - **live, separately gated one-shot operator entrypoint** for exact Attempt 008 API/UI suspension; it is excluded from aggregate checks and grants no generic process/Docker authority, cleanup, retry, release, or UAT claim.
