...
- [docs/codex/local-v1-lv1-003-o4-execution-authorization.md](docs/codex/local-v1-lv1-003-o4-execution-authorization.md) - prepared one-shot Attempt 012 authorization for the reviewed closed thirteen-key fixed-Node state projection. It preserves consumed Attempts 001-011 and recovery history, binds the exact six-path authorization child and future annotated review tag, grants only the existing producer, closed Docker lifecycle, Hermes, provider, and O4 evidence fields, and keeps the other 14 authority fields false. Preparation is not execution: the gate remains invalid and the live target refuses before activity until the exact candidate receives independent review and the fixed nonforce annotated tag; release and UAT remain false.
...
- `make local-v1-lv1-003-o4-execution-authorization-check` - validate the prepared Attempt 012 exact six-path, single-parent authorization child, reviewed fixed-Node state-projection digests, immutable Attempt 011 disposition and receipts, future annotated review tag, one-shot budget, no retry, exactly five bounded live fields, and 14 false authority fields. Before the exact tag exists, the check fails closed without live activity.
...
- `make local-v1-lv1-003-o4-producer-run` - retained **live, gate-protected one-shot Attempt 012 operator entrypoint** using `uv run python -m scripts.local_v1_lv1_003_o4_producer`. The gate consumes the fresh attempt before any activity and currently refuses because the exact annotated review tag does not exist. Any invocation consumes Attempt 012 and requires an immediate separate exact disposition; no retry follows success or failure. The producer contract and tag by themselves grant no execution authority. This target is not part of release, milestone, or static checks.
