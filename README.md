...
- [docs/codex/local-v1-lv1-003-o4-attempt008-port-release-authorization.md](docs/codex/local-v1-lv1-003-o4-attempt008-port-release-authorization.md) - separate unconsumed Attempt 002 one-shot authority for stopping only the known Attempt 008 API/UI containers, with an exact Attempt 001 closure prerequisite, descriptor-anchored private sealed Docker copy, macOS-aware stable inherited-group evidence, conservative invalid-lane consumption state, closed journal/disposition crosslinks, race-resistant receipt reads, and honest observed-unchanged versus not-targeted preservation semantics.
- `make local-v1-lv1-003-o4-attempt008-port-release-check` - run the fake-only authorization, executable-identity, sealed-command, journal, receipt-race, preservation, and negative tests without Docker or live access.
- `make local-v1-lv1-003-o4-attempt008-port-release-run` - **live, separately gated Attempt 002 one-shot operator entrypoint** for exact Attempt 008 API/UI suspension; it is excluded from aggregate checks and grants no generic process/Docker authority, cleanup, retry, release, or UAT claim.
