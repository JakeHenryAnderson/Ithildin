...
- [docs/codex/local-v1-lv1-003-o4-execution-authorization.md](docs/codex/local-v1-lv1-003-o4-execution-authorization.md) - one-shot Attempt 005 authorization over the exact diagnostic repair and exact retained Attempt 001-004 and recovery evidence; only the five necessary O4 execution authorities are true for the dynamically validated child, while credential, arbitrary host/process/shell/general Docker, new-power/tool, evidence-deletion, release, production, promotion, and UAT authority remain false.
- [docs/codex/local-v1-lv1-003-o4-producer-contract.md](docs/codex/local-v1-lv1-003-o4-producer-contract.md) - producer-and-assembler contract with the exact gate-protected module invocation, fixed command/API vocabulary, actual Gateway truth, ambiguous-enrollment and unconfirmed-revocation retention, durable report rollback, bounded-inventory requirements, and the explicit same-UID Docker-context mutation nonclaim; the producer contract itself grants no execution authority, while the separate Attempt 005 gate grants one supervised invocation only.
...
- `make local-v1-lv1-003-o4-execution-authorization-check` - validate the exact seven-path Attempt 005 child, diagnostic-repair review lineage, retained Attempt 001-004 and recovery evidence, one attempt budget, no retry, and the closed five-field execution authority.
...
- `make local-v1-lv1-003-o4-producer-run` - **live, gate-protected operator entrypoint** using `uv run python -m scripts.local_v1_lv1_003_o4_producer`; Attempt 005 permits exactly one central-manager-supervised invocation only on the exact clean child, with no automatic retry. It is not part of release, milestone, or static checks.
