...
- [docs/codex/local-v1-lv1-003-o4-execution-authorization.md](docs/codex/local-v1-lv1-003-o4-execution-authorization.md) - prepared Attempt 017 exact six-path, one-shot authorization gate preserving immutable consumed Attempts 001-016 and binding the reviewed Gateway mission-convergence implementation. The real pre-tag checkout remains fail-closed with zero effective budget and all live authority false.
...
- `make local-v1-lv1-003-o4-execution-authorization-check` - validate the Attempt 017 clean exact six-path child, reviewed seven-path Gateway mission-convergence implementation/review-record bytes, immutable consumed Attempts 001-016 and private receipt bindings through Attempt 016, exact annotated Attempt 017 tag, budget one, and exactly five bounded live authority fields. Before the tag and exact candidate exist, it reports invalid, budget zero, and all live authority false.
...
- `make local-v1-lv1-003-o4-producer-run` - **prepared gate-protected one-shot Attempt 017 entrypoint** using `uv run python -m scripts.local_v1_lv1_003_o4_producer`. It refuses before activity unless the checkout is the clean exact reviewed candidate with the fixed annotated tag; no concurrent, automatic, or post-attempt retry is authorized, and immediate consumed disposition is mandatory. This target remains outside release, milestone, and static checks.
