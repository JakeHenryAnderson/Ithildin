...
- [docs/codex/local-v1-lv1-003-o4-execution-authorization.md](docs/codex/local-v1-lv1-003-o4-execution-authorization.md) - prepared Attempt 021 one-shot gate binding the exact five-path Gateway session-binding repair, durable `GO` review record, exact six-path candidate, required annotated review tag, and immutable consumed Attempts 001-020; before the tag exists the effective budget zero posture keeps all 19 authority fields false.
...
- `make local-v1-lv1-003-o4-execution-authorization-check` - validate the prepared Attempt 021 exact six-path candidate, exact reviewed Gateway session-binding repair and durable review record, immutable consumed Attempts 001-020, required annotated review tag, one-shot budget, five-field maximum live-authority ceiling, and immediate consumed-disposition requirement. Before the exact tag exists it fails closed solely for the tag with effective budget zero and all 19 authority fields are false.
...
- `make local-v1-lv1-003-o4-producer-run` - **prepared gate-protected one-shot Attempt 021 entrypoint** using `uv run python -m scripts.local_v1_lv1_003_o4_producer`. It refuses before activity until the exact six-path candidate has its required annotated review tag, remains outside release, milestone, and static checks, and requires an immediate consumed disposition after the single central-manager invocation.
