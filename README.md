...
- [docs/codex/local-v1-lv1-003-o4-execution-authorization.md](docs/codex/local-v1-lv1-003-o4-execution-authorization.md) - consumed Attempt 012 closure preserving immutable Attempts 001-011, the exact reviewed execution candidate/tag, the domain-separated private run/project bindings, the closed thirteen-key sequential fixed-Node projection, bounded receipt metadata, and zero live authority. Retry, recovery, cleanup, successor, release, and UAT authority remain false.
...
- `make local-v1-lv1-003-o4-execution-authorization-check` - validate the consumed Attempt 012 exact eight-path closure child, immutable reviewed execution tag/candidate, domain-separated owner-only receipt selection, exact receipt and thirteen-key diagnostic bindings, zero attempt budget, and all 19 authority fields false.
...
- `make local-v1-lv1-003-o4-producer-run` - retained **closed Attempt 012 operator entrypoint** using `uv run python -m scripts.local_v1_lv1_003_o4_producer`. Its fail-closed gate now refuses before activity because Attempt 012 is consumed with budget zero and no retry, recovery, cleanup, or successor authority. This target remains outside release, milestone, and static checks.
