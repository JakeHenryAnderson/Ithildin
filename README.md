...
- [docs/codex/local-v1-lv1-003-o4-execution-authorization.md](docs/codex/local-v1-lv1-003-o4-execution-authorization.md) - closed consumed Attempt 019 gate preserving immutable consumed Attempts 001-019, budget zero, all 19 authority fields false, the bounded identity-free Node projection `completion_recorded` / `runner_reported_succeeded`, and control-flow-only evidence that the terminal mission gate passed before `gateway_run_detail_invalid`.
...
- `make local-v1-lv1-003-o4-execution-authorization-check` - validate the closed consumed Attempt 019 candidate/tag binding, immutable consumed Attempts 001-019, exact private receipt digests and identity-free Node projection, control-flow-only terminal mission gate evidence, budget zero, recorded immediate consumed disposition, and all 19 authority fields false.
...
- `make local-v1-lv1-003-o4-producer-run` - **closed consumed Attempt 019 entrypoint** using `uv run python -m scripts.local_v1_lv1_003_o4_producer`. It refuses before activity with budget zero and no live authority, remains outside release, milestone, and static checks, and grants no Attempt 020, repair, retry, or execution authority.
