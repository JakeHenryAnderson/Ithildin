...
- [docs/codex/local-v1-lv1-003-o4-execution-authorization.md](docs/codex/local-v1-lv1-003-o4-execution-authorization.md) - closed consumed Attempt 014 authorization record preserving immutable Attempts 001-013, binding the exact tagged candidate and private receipts without reproducing identities, recording phase `socket_parent_validation_entered`, and keeping all 19 authority fields false with budget zero.
...
- `make local-v1-lv1-003-o4-execution-authorization-check` - validate the consumed Attempt 014 exact eight-path closure child, immutable tagged execution candidate, domain-separated owner-only receipt selection, exact receipt and fourteen-key diagnostic bindings, budget zero, and all 19 authority fields false.
...
- `make local-v1-lv1-003-o4-producer-run` - retained **closed consumed Attempt 014 entrypoint** using `uv run python -m scripts.local_v1_lv1_003_o4_producer`. Its fail-closed gate refuses before activity because the one-shot budget is zero and the attempt is consumed. No retry, automatic retry, recovery, cleanup, successor attempt, release, promotion, production, or UAT authority exists. This target remains outside release, milestone, and static checks.
