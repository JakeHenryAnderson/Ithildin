...
- [docs/codex/local-v1-lv1-003-o4-execution-authorization.md](docs/codex/local-v1-lv1-003-o4-execution-authorization.md) - exact Attempt 004 immediate-child gate with one central-manager-supervised invocation, no retry, retained Attempt 001-003 and recovery lineage, and all release, production, promotion, credential-custody, and UAT authority false.
- [docs/codex/local-v1-lv1-003-o4-producer-contract.md](docs/codex/local-v1-lv1-003-o4-producer-contract.md) - producer-and-assembler contract with the exact gate-protected module invocation, fixed command/API vocabulary, actual Gateway truth, ambiguous-enrollment and unconfirmed-revocation retention, durable report rollback, bounded-inventory requirements, and the explicit same-UID Docker-context mutation nonclaim; the producer contract itself grants no execution authority, while one supervised invocation is available only through the separate exact Attempt 004 gate.
...
- `make local-v1-lv1-003-o4-execution-authorization-check` - validate the exact Attempt 004 immediate-child binding, runtime-native bridge/producer review, fixed profile/source/provider contract, retained Attempt 001-003 and recovery evidence, one-attempt budget, no retry, and fail-closed non-O4 authority.
...
- `make local-v1-lv1-003-o4-producer-run` - **live, gate-protected operator entrypoint** using `uv run python -m scripts.local_v1_lv1_003_o4_producer`; it is authorized only on the exact clean Attempt 004 immediate child, for one central-manager-supervised invocation with no retry, and otherwise refuses before live work. It is not part of release, milestone, or static checks.
