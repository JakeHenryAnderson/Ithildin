...
- [docs/codex/local-v1-lv1-003-o4-execution-authorization.md](docs/codex/local-v1-lv1-003-o4-execution-authorization.md) - Attempt 010 consumed closure for exact `fixed_node_start_failed` after stage 11, with cleanup complete under reviewed producer semantics, zero attempt budget, all 19 authority fields false, and no retry, recovery, cleanup, or successor authority. It preserves the Attempt 009 consumed closure, retained Attempt 001-009 and recovery evidence, exact Attempt 009 runtime-root absence, and no port/process or general Docker/runtime claim. The predecessor Attempt 008 recovery is closed, but this closure does not derive successor authority from recovery; release and UAT remain false.
...
- `make local-v1-lv1-003-o4-execution-authorization-check` - validate the consumed Attempt 010 exact-child closure, immutable descendant lineage, current MCC authorization, exact eight-path candidate, retained Attempt 001-009 history, closed tracked Attempt 008 recovery binding, exact-candidate review tag, fixed receipt bindings, zero attempt budget, no retry, and all 19 authority fields false.
...
- `make local-v1-lv1-003-o4-producer-run` - **live, gate-protected operator entrypoint** using `uv run python -m scripts.local_v1_lv1_003_o4_producer`; Attempt 010 is consumed and the current closure always refuses execution. The producer contract itself grants no execution authority. It is not part of release, milestone, or static checks.
