...
- [docs/codex/local-v1-lv1-003-o4-execution-authorization.md](docs/codex/local-v1-lv1-003-o4-execution-authorization.md) - Attempt 010 one-shot candidate for one fresh isolated attempt, not a retry, recovery, or cleanup; it preserves the Attempt 009 consumed closure, retained Attempt 001-009 and recovery evidence, exact Attempt 009 runtime-root absence, and no port/process or general Docker/runtime claim. The predecessor Attempt 008 recovery is closed, but this candidate does not derive successor authority from recovery. Exactly five bounded live authority fields are true and the remaining 14 authority fields false; independent exact review of the frozen candidate is required, and release and UAT remain false.
...
- `make local-v1-lv1-003-o4-execution-authorization-check` - validate the fresh Attempt 010 exact-child one-shot contract, current MCC authorization, exact seven-path candidate, retained Attempt 001-009 history, closed tracked Attempt 008 recovery binding, exact-candidate independent-review tag, one-attempt budget, no retry, and the five-true/fourteen-false authority boundary.
...
- `make local-v1-lv1-003-o4-producer-run` - **live, gate-protected operator entrypoint** using `uv run python -m scripts.local_v1_lv1_003_o4_producer`; Attempt 010 permits exactly one central-manager supervised invocation only after the frozen exact candidate receives independent exact review. Implementation preparation alone grants no live execution. The producer contract itself grants no execution authority. It is not part of release, milestone, or static checks.
