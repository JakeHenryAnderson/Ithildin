...
- [docs/codex/local-v1-lv1-003-o4-execution-authorization.md](docs/codex/local-v1-lv1-003-o4-execution-authorization.md) - fresh one-shot Attempt 011 authorization from the consumed Attempt 010 `fixed_node_start_failed` closure. Before the exact annotated review tag, the gate collapses to zero attempt budget and all 19 authority fields false; after exact tag peel, only the existing five bounded live fields are true and the other 14 remain false. It preserves retained Attempt 001-010 and recovery evidence, grants no automatic retry, and confirms that release and UAT remain false.
...
- `make local-v1-lv1-003-o4-execution-authorization-check` - validate the fresh Attempt 011 exact seven-path child, retained Attempt 001-010 history and recovery evidence, fixed diagnostic review and runtime parity, annotated exact-candidate review tag, one-shot budget, no retry, five bounded live fields, and 14 false authority fields. Without the exact tag it fails closed with budget zero and all live authority false.
...
- `make local-v1-lv1-003-o4-producer-run` - **live, gate-protected one-shot Attempt 011 operator entrypoint** using `uv run python -m scripts.local_v1_lv1_003_o4_producer`. It refuses until the clean exact child is bound by the annotated `ithildin/lv1-003-o4-attempt011-reviewed` tag; any invocation consumes Attempt 011 and requires immediate disposition. The producer contract itself grants no execution authority. It is not part of release, milestone, or static checks.
