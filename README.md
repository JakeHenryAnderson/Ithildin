...
- [docs/codex/local-v1-lv1-003-o4-execution-authorization.md](docs/codex/local-v1-lv1-003-o4-execution-authorization.md) - prepared Attempt 019 one-shot gate binding the committed two-operation terminal repair review, exact six-path candidate, annotated review tag, budget one only after exact review, and the retained five-field maximum live-authority ceiling; immutable consumed Attempts 001-018 remain closed with bounded identity-free Gateway and Node receipt projections including terminal reason `bridge_disconnected`.
...
- `make local-v1-lv1-003-o4-execution-authorization-check` - validate the prepared Attempt 019 exact six-path candidate, committed two-operation terminal repair review, required annotated tag, immutable consumed Attempts 001-018, one-shot budget, five-field maximum live-authority ceiling, and immediate consumed-disposition requirement. Before the tag exists it fails closed with effective budget zero and all 19 authority fields false.
...
- `make local-v1-lv1-003-o4-producer-run` - **prepared gate-protected one-shot Attempt 019 entrypoint** using `uv run python -m scripts.local_v1_lv1_003_o4_producer`. It refuses before activity until the exact candidate has its required annotated review tag, remains outside release, milestone, and static checks, and requires an immediate consumed disposition after the single central-manager invocation.
