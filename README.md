...
- [docs/codex/local-v1-lv1-003-o4-execution-authorization.md](docs/codex/local-v1-lv1-003-o4-execution-authorization.md) - one-shot Attempt 006 exact-child authorization with a single supervised invocation budget, exact retained Attempt 001-005 and recovery evidence, a reviewed bounded API container-state diagnostic without a root-cause or success claim, exactly five live execution fields true, and all recovery, evidence-deletion, release, production, promotion, credential-custody, and UAT authority false.
- [docs/codex/local-v1-lv1-003-o4-producer-contract.md](docs/codex/local-v1-lv1-003-o4-producer-contract.md) - producer-and-assembler contract with the exact gate-protected module invocation, fixed command/API vocabulary, actual Gateway truth, ambiguous-enrollment and unconfirmed-revocation retention, durable report rollback, bounded-inventory requirements, and the explicit same-UID Docker-context mutation nonclaim; the producer contract itself grants no execution authority, while the separate Attempt 006 exact-child gate grants only the bounded one-shot authority it enumerates.
...
- `make local-v1-lv1-003-o4-execution-authorization-check` - validate the Attempt 006 clean immediate-child seven-path binding, retained Attempt 001-005 and recovery evidence, budget one, no retry, and exactly five live execution fields true.
...
- `make local-v1-lv1-003-o4-producer-run` - **live, gate-protected operator entrypoint** using `uv run python -m scripts.local_v1_lv1_003_o4_producer`; Attempt 006 permits exactly one central-manager-supervised invocation only after the exact-child gate passes. It is not part of release, milestone, or static checks.
