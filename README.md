...
disposition. `LV1-004` is the ordered next milestone, but it is paused and not started; `O5`,
release, and UAT authority remain false. The
...
- [docs/codex/local-v1-lv1-004-failure-recovery.md](docs/codex/local-v1-lv1-004-failure-recovery.md) - active bounded implementation contract for one candidate-bound restart/replay/partition/revocation/stale-configuration/manual-rollback scenario; no O5 closure, release, or UAT claim yet.
...
- `make local-v1-failure-recovery-run` - run the clean-candidate isolated loopback LV1-004 recovery journey. It starts no Docker service, runner, provider, or arbitrary process and writes only ignored local evidence.
