...
- [docs/codex/local-v1-lv1-003-o4-execution-authorization.md](docs/codex/local-v1-lv1-003-o4-execution-authorization.md) - consumed Attempt 011 closure for `fixed_node_start_failed` with normalized `fixed_node_runtime_state_inconsistent`, cleanup complete under reviewed semantics, zero budget, all 19 authority fields false, and no retry, recovery, cleanup, or successor authority. It preserves Attempts 001-010, recovery history, and the rejected `af88dbf` lineage; release and UAT remain false.
...
- `make local-v1-lv1-003-o4-execution-authorization-check` - validate the consumed Attempt 011 exact eight-path closure, immutable candidate/tag lineage, retained Attempt 001-010 and recovery evidence, normalized diagnostic and receipt bindings, zero budget, no retry, and all 19 authority fields false.
...
- `make local-v1-lv1-003-o4-producer-run` - retained **live, gate-protected operator entrypoint** using `uv run python -m scripts.local_v1_lv1_003_o4_producer`; Attempt 011 is consumed and the current closure always refuses execution. The producer contract itself grants no execution authority. It is not part of release, milestone, or static checks.
