...
from scripts import (
    ...
    v1_rc_packet,
    v1_rc_post_review_triage_check,
...
def test_v1_rc_roadmap_is_wired() -> None:
    ...
    post_review_triage = Path("docs/codex/v1.0-rc-post-review-triage.md").read_text(
        encoding="utf-8"
    )
    ...
    assert quickstart_report["tool_count"] == 24
    ...
    assert workbench_report["tool_count"] == 24
    assert workbench_report["runtime_changes_allowed"] is False
    assert workbench_report["new_power_classes_allowed"] is False
    assert assurance_report["valid"] is True
    assert assurance_report["tool_count"] == 24
    assert assurance_report["open_critical_high_findings"] == 0
    assert assurance_report["pending_external_review_rows"] == 12
    ...
    assert post_review_triage_report["valid"] is True
    assert post_review_triage_report["tool_count"] == 24
    assert post_review_triage_report["selected_capability"] == "not selected"
    assert post_review_triage_report["capability_expansion_allowed"] is False
    assert (
        post_review_triage_report["public_security_product_positioning_allowed"]
        is False
    )
    ...
    for phrase in [
        ...
        assert phrase in post_review_triage
    for phrase in [
...
def test_v1_rc_packet_includes_current_artifact_map(tmp_path: Path) -> None:
    ...
    post_review_triage = output_dir.joinpath("13_V1_RC_POST_REVIEW_TRIAGE.md").read_text(
        encoding="utf-8"
    )
    artifacts = output_dir.joinpath("14_V1_RC_ARTIFACTS.md").read_text(encoding="utf-8")
    commands = output_dir.joinpath("15_V1_RC_COMMANDS.md").read_text(encoding="utf-8")
    hashes = json.loads(
    ...
    assert "Ithildin v1.0 RC Post-Review Triage" in post_review_triage
...
def test_mission_control_side_handoff_plan_is_wired() -> None:
    report = mission_control_side_handoff_plan_check.build_report(Path.cwd())
    doc = Path("docs/codex/mission-control-side-handoff-plan.md").read_text(
        encoding="utf-8"
    )
    readme = Path("README.md").read_text(encoding="utf-8")
    makefile = Path("Makefile").read_text(encoding="utf-8")
    ...
    assert report["mission_control_approval_authority_allowed"] is False
    assert report["mission_control_audit_authority_allowed"] is False
    assert report["mission_control_side_runtime_implementation_allowed"] is False
    assert report["local_model_invocation_allowed"] is False
    assert report["sandbox_orchestration_allowed"] is False
    assert report["trusted_host_promotion_allowed"] is False
    assert report["new_power_classes_allowed"] is False
    for phrase in [
        ...
        "Mission Control-side work order",
    ...
    for forbidden in [
        ...
        assert forbidden not in doc
    assert "make mission-control-side-handoff-plan-check" in readme
    assert "mission-control-side-handoff-plan-check:" in makefile
    assert "mission-control-side-handoff-plan-check" in release_check_body
    assert "mission-control-side-handoff-plan-check" in (
        release_guardrails.REQUIRED_RELEASE_CHECK_FRAGMENTS
    )
    assert "docs/codex/mission-control-side-handoff-plan.md" in docs_site
    assert "docs/codex/mission-control-side-handoff-plan.md" in review_docs.REVIEW_DOCS
...
def test_mission_control_handoff_negative_fixtures_are_wired() -> None:
    report = mission_control_handoff_negative_fixtures_check.build_report(Path.cwd())
    readme = Path("README.md").read_text(encoding="utf-8")
    makefile = Path("Makefile").read_text(encoding="utf-8")
    docs_site = Path("scripts/build_docs_site.py").read_text(encoding="utf-8")
    review_index = Path("docs/codex/review-docs-index.md").read_text(encoding="utf-8")
    ...
    assert report["trusted_host_promotion_allowed"] is False
    assert report["new_power_classes_allowed"] is False
    assert "make mission-control-handoff-negative-fixtures-check" in readme
    assert "mission-control-handoff-negative-fixtures-check:" in makefile
    assert "mission-control-handoff-negative-fixtures-check" in release_check_body
    assert "mission-control-handoff-negative-fixtures-check" in (
        release_guardrails.REQUIRED_RELEASE_CHECK_FRAGMENTS
    )
...
def test_sandbox_vm_worker_boundary_charter_is_wired() -> None:
    report = sandbox_vm_worker_boundary_charter_check.build_report(Path.cwd())
    readme = Path("README.md").read_text(encoding="utf-8")
    makefile = Path("Makefile").read_text(encoding="utf-8")
    docs_site = Path("scripts/build_docs_site.py").read_text(encoding="utf-8")
    review_index = Path("docs/codex/review-docs-index.md").read_text(encoding="utf-8")
    ...
    assert report["trusted_host_promotion_allowed"] is False
    assert report["new_power_classes_allowed"] is False
    assert "make sandbox-vm-worker-boundary-charter-check" in readme
    assert "sandbox-vm-worker-boundary-charter-check:" in makefile
    assert "sandbox-vm-worker-boundary-charter-check" in release_check_body
    assert "sandbox-vm-worker-boundary-charter-check" in (
        release_guardrails.REQUIRED_RELEASE_CHECK_FRAGMENTS
    )
    assert "docs/codex/sandbox-vm-worker-boundary-charter.md" in docs_site
    assert "docs/codex/sandbox-vm-worker-boundary-charter.md" in review_docs.REVIEW_DOCS
...
def test_sandbox_vm_preflight_contract_is_wired() -> None:
    report = sandbox_vm_preflight_contract_check.build_report(Path.cwd())
    readme = Path("README.md").read_text(encoding="utf-8")
    makefile = Path("Makefile").read_text(encoding="utf-8")
    docs_site = Path("scripts/build_docs_site.py").read_text(encoding="utf-8")
    review_index = Path("docs/codex/review-docs-index.md").read_text(encoding="utf-8")
    ...
    assert report["new_power_classes_allowed"] is False
    assert "make sandbox-vm-preflight-contract-check" in readme
    assert "sandbox-vm-preflight-contract-check:" in makefile
    assert "sandbox-vm-preflight-contract-check" in release_check_body
    assert "sandbox-vm-preflight-contract-check" in (
        release_guardrails.REQUIRED_RELEASE_CHECK_FRAGMENTS
    )
    assert "docs/codex/sandbox-vm-preflight-contract.md" in docs_site
    assert "docs/codex/sandbox-vm-preflight-contract.md" in review_docs.REVIEW_DOCS
    assert "Sandbox/VM Preflight Contract" in review_index
...
def test_sandbox_vm_static_profile_preflight_plan_is_wired() -> None:
    report = sandbox_vm_static_profile_preflight_plan_check.build_report(Path.cwd())
    readme = Path("README.md").read_text(encoding="utf-8")
    makefile = Path("Makefile").read_text(encoding="utf-8")
    docs_site = Path("scripts/build_docs_site.py").read_text(encoding="utf-8")
    enterprise = Path("docs/codex/enterprise-readiness-runway.md").read_text(
    ...
    plan = Path("docs/codex/sandbox-vm-static-profile-preflight-plan.md").read_text(
        encoding="utf-8"
    )
    release_check_body = makefile.partition("release-check:")[2].partition("\n\n")[0]
    ...
    assert report["network_expansion_allowed"] is False
    assert report["new_power_classes_allowed"] is False
    assert "make sandbox-vm-static-profile-preflight-plan-check" in readme
    assert "sandbox-vm-static-profile-preflight-plan-check:" in makefile
    assert "sandbox-vm-static-profile-preflight-plan-check" in release_check_body
    assert "sandbox-vm-static-profile-preflight-plan-check" in (
        release_guardrails.REQUIRED_RELEASE_CHECK_FRAGMENTS
    )
    assert "docs/codex/sandbox-vm-static-profile-preflight-plan.md" in docs_site
    assert (
...
def test_sandbox_vm_static_preflight_source_review_packet_is_wired(
    ...
    assert "sandbox-vm-static-preflight-disposition-plan.md" in contracts
...
def test_sandbox_vm_static_preflight_external_review_bundle_is_wired(
    tmp_path: Path,
) -> None:
    report = sandbox_vm_static_preflight_external_review_bundle.build_check_report(Path.cwd())
    output_dir = tmp_path / "sandbox-vm-static-preflight-external-review"
    sandbox_vm_static_preflight_external_review_bundle.build_bundle(
        repo_root=Path.cwd(),
        output_dir=output_dir,
        allow_dirty=True,
        run_commands=False,
    ...
    index = (
        output_dir / "00_SANDBOX_VM_STATIC_PREFLIGHT_EXTERNAL_REVIEW_INDEX.md"
    ).read_text(encoding="utf-8")
    prompt = (
        output_dir / "01_SANDBOX_VM_STATIC_PREFLIGHT_EXTERNAL_REVIEW_PROMPT.md"
    ).read_text(encoding="utf-8")
    source_packet = (
        output_dir / "02_SANDBOX_VM_STATIC_PREFLIGHT_SOURCE_PACKET.md"
    ).read_text(encoding="utf-8")
    disposition_packet = (
        output_dir / "03_SANDBOX_VM_STATIC_PREFLIGHT_DISPOSITION_PACKET.md"
    ).read_text(encoding="utf-8")
    contracts = (
        output_dir / "04_SANDBOX_VM_STATIC_PREFLIGHT_IMPLEMENTATION_CONTRACTS.md"
    ).read_text(encoding="utf-8")
    fixtures = (
        output_dir / "05_SANDBOX_VM_STATIC_PREFLIGHT_FIXTURES_NEGATIVES.md"
    ).read_text(encoding="utf-8")
    response = (
        output_dir / "06_SANDBOX_VM_STATIC_PREFLIGHT_RESPONSE_CLOSURE_TRIAGE.md"
    ).read_text(encoding="utf-8")
    reproduction = (
        output_dir / "07_SANDBOX_VM_STATIC_PREFLIGHT_REPRODUCTION_QUEUE_STATUS.md"
    ).read_text(encoding="utf-8")
    evidence = (
        output_dir / "08_SANDBOX_VM_STATIC_PREFLIGHT_COMMAND_EVIDENCE.md"
    ).read_text(encoding="utf-8")
    readme = Path("README.md").read_text(encoding="utf-8")
    makefile = Path("Makefile").read_text(encoding="utf-8")
    docs_site = Path("scripts/build_docs_site.py").read_text(encoding="utf-8")
    ...
    assert "does not close `ERG-003`" in index
    assert "Finding namespace: `EXT-SVP-###`" in prompt
    assert "Can `ERG-003` move from `external_review_required`" in prompt
    assert "Do not approve live VM/container inspection" in prompt
    assert "02_SANDBOX_VM_STATIC_PREFLIGHT_CONTRACTS.md" in source_packet
    assert "01_SANDBOX_VM_STATIC_PREFLIGHT_DISPOSITION_PROMPT.md" in disposition_packet
    assert "sandbox-vm-static-preflight-implementation-decision.md" in contracts
    assert "v3-sandbox-vm-static-preflight-internal-review.md" in contracts
    assert "sandbox-vm-static-profile.local-preview.example.json" in fixtures
    ...
    assert "enterprise-external-review-queue.md" in reproduction
    assert "Recommended next review" in reproduction
    assert "sandbox-vm-live-poc-preconditions-map.md" in reproduction
    for flag in [
        '"runtime_changes_allowed": false',
        '"live_vm_inspection_allowed": false',
        '"mission_control_runtime_allowed": false',
        ...
        assert flag in evidence
    assert "make sandbox-vm-static-preflight-external-review-bundle" in readme
    assert "sandbox-vm-static-preflight-external-review-bundle:" in makefile
    assert "sandbox-vm-static-preflight-external-review-bundle-check:" in makefile
    assert "sandbox-vm-static-preflight-external-review-bundle-check" in release_check_body
    assert "$(MAKE) sandbox-vm-static-preflight-external-review-bundle" in (
        review_candidate_body
    )
    assert "sandbox-vm-static-preflight-external-review-bundle-check" in (
        release_guardrails.REQUIRED_RELEASE_CHECK_FRAGMENTS
    )
    assert "$(MAKE) sandbox-vm-static-preflight-external-review-bundle" in (
        release_guardrails.REQUIRED_REVIEW_CANDIDATE_STEPS
    )
    assert "docs/codex/sandbox-vm-static-preflight-external-review-bundle.md" in docs_site
    assert (
        "docs/codex/sandbox-vm-static-preflight-external-review-bundle.md"
        in review_docs.REVIEW_DOCS
    )
    assert "Sandbox/VM Static Preflight External Review Bundle" in review_index
...
def test_sandbox_vm_static_preflight_disposition_plan_is_wired() -> None:
    report = sandbox_vm_static_preflight_disposition_plan_check.build_report(Path.cwd())
    doc = Path("docs/codex/sandbox-vm-static-preflight-disposition-plan.md").read_text(
        encoding="utf-8"
    )
    readme = Path("README.md").read_text(encoding="utf-8")
    makefile = Path("Makefile").read_text(encoding="utf-8")
    ...
    for blocked in [
        ...
        assert blocked in doc
    assert "make sandbox-vm-static-preflight-disposition-plan-check" in readme
    assert "sandbox-vm-static-preflight-disposition-plan-check:" in makefile
    assert "sandbox-vm-static-preflight-disposition-plan-check" in release_check_body
    assert "sandbox-vm-static-preflight-disposition-plan-check" in (
        release_guardrails.REQUIRED_RELEASE_CHECK_FRAGMENTS
    )
    assert "docs/codex/sandbox-vm-static-preflight-disposition-plan.md" in docs_site
    assert (
...
def test_sandbox_vm_live_poc_evidence_contract_is_wired() -> None:
    report = sandbox_vm_live_poc_evidence_contract_check.build_report(Path.cwd())
    doc = Path("docs/codex/sandbox-vm-live-poc-evidence-contract.md").read_text(
        encoding="utf-8"
    )
    readme = Path("README.md").read_text(encoding="utf-8")
    makefile = Path("Makefile").read_text(encoding="utf-8")
    ...
    assert report["new_power_classes_allowed"] is False
    for phrase in [
        "Status: design-only evidence contract for a future `ERG-004`",
    ...
    for blocked in [
        ...
        assert blocked in doc
    assert "make sandbox-vm-live-poc-evidence-contract-check" in readme
    assert "sandbox-vm-live-poc-evidence-contract-check:" in makefile
    assert "sandbox-vm-live-poc-evidence-contract-check" in release_check_body
    assert "sandbox-vm-live-poc-evidence-contract-check" in (
        release_guardrails.REQUIRED_RELEASE_CHECK_FRAGMENTS
    )
    assert "docs/codex/sandbox-vm-live-poc-evidence-contract.md" in docs_site
...
def test_sandbox_vm_live_poc_response_dry_run_is_wired() -> None:
    report = sandbox_vm_live_poc_response_dry_run.run_dry_run(Path.cwd())
    doc = Path("docs/codex/sandbox-vm-live-poc-response-dry-run.md").read_text(
        encoding="utf-8"
    )
    readme = Path("README.md").read_text(encoding="utf-8")
    makefile = Path("Makefile").read_text(encoding="utf-8")
    ...
    assert report["valid"] is True
    assert report["dry_run_doc"] == "docs/codex/sandbox-vm-live-poc-response-dry-run.md"
    assert (
    ...
    for phrase in [
        ...
        assert phrase not in doc
    assert "make sandbox-vm-live-poc-response-dry-run" in readme
    assert "sandbox-vm-live-poc-response-dry-run:" in makefile
    assert (
        "sandbox-vm-live-poc-response-dry-run" in release_check_body
        or "release-check: sandbox-vm-live-poc-response-dry-run" in makefile
    )
    assert "sandbox-vm-live-poc-response-dry-run" in (
        release_guardrails.REQUIRED_RELEASE_CHECK_FRAGMENTS
    )
    assert "docs/codex/sandbox-vm-live-poc-response-dry-run.md" in docs_site
    assert "docs/codex/sandbox-vm-live-poc-response-dry-run.md" in (
        review_docs.REVIEW_DOCS
    )
    assert "Sandbox/VM Live POC Response Dry Run" in review_index
    assert "sandbox-vm-live-poc-response-dry-run.md" in enterprise
    assert "sandbox-vm-live-poc-response-dry-run.md" in gap_matrix
...
def test_v08_capability_design_gate_is_wired() -> None:
    ...
    assert report["evidence"]["tool_count"] == 24
...
def test_read_only_metadata_capability_check_is_wired() -> None:
    ...
    assert "Tool count is `24`" in debt_register
...
def test_project_config_summary_proposal_check_is_wired() -> None:
    ...
    for phrase in [
        ...
        assert phrase in proposal
    assert "tool count remains `24`" in selection
    assert "implementation remains blocked" in selection
...
def test_project_risk_summary_proposal_and_plan_are_wired() -> None:
    ...
    assert proposal_report["evidence"]["tool_count"] == 24
    ...
    assert plan_report["evidence"]["tool_count"] == 24
    ...
    assert preimplementation_report["tool_count"] == 24
...
def test_project_release_summary_preimplementation_check_is_wired() -> None:
    ...
    assert report["valid"] is True
    assert report["proposal"] == "project.release.summary"
    assert report["scope"] == "fixture_contract_after_implementation"
    ...
    assert report["tool_count"] == 24
    assert report["new_power_classes_allowed"] is False
    assert report["evidence"]["tool_surface"]["tool_count"] == 24
...
def test_project_release_summary_implementation_gate_is_wired() -> None:
    ...
    assert report["implementation_status"] == "implemented_limited_read_only"
...
def test_project_release_summary_design_review_packet_builds_from_fixture(
    ...
    assert preimplementation_json["tool_count"] == 24
    ...
    assert review_handoff_check_json["tool_count"] == 24
    assert (
    ...
    assert "Status: fixture/test contract retained after implementation" in fixture_doc
    ...
    assert "Tool count: `24`" in index_doc
...
def test_project_release_summary_review_handoff_check_is_wired() -> None:
    ...
    assert "Current tool count remains `24`" in source_review
...
def test_project_release_summary_source_review_bundle_builds_from_fixture(
    ...
    assert "Status: implemented source-review handoff" in index
    ...
    assert gate_evidence["tool_surface"]["tool_count"] == 24
...
def test_tool_surface_invariant_gate_is_wired_and_valid() -> None:
    ...
    assert report["tool_count"] == 24
    assert report["manifest_file_count"] == 24
...
def test_external_response_normalization_accepts_lane_specific_ids() -> None:
    ...
    siem_export_adapter_response = "\n".join(
        [
            "# SIEM Export Adapter Review",
            "",
            "| Finding ID | Severity | Area | Affected files/functions | "
            "Blocking status | Disposition | Recommended fix |",
            "| --- | --- | --- | --- | --- | --- | --- |",
            "| EXT-SIEM-ADAPTER-001 | medium | siem-export-adapter | "
            "docs/codex/siem-export-adapter-disposition-packet.md | "
            "should-fix | open | clarify adapter disposition wording |",
        ]
    )
    siem_export_adapter_normalized = external_response_normalize.normalize_response(
        siem_export_adapter_response,
        reviewer="GPT 5.5 Pro",
        reviewer_type="external-model",
        source_access="packet-and-source",
    ...
    assert (
        siem_export_adapter_normalized["findings"][0]["finding_id"]
        == "EXT-SIEM-ADAPTER-001"
    )
    assert (
        siem_export_adapter_normalized["findings"][0]["area"]
        == "siem-export-adapter"
    )
...
def test_siem_export_adapter_external_response_intake_is_wired() -> None:
    report = siem_export_adapter_external_response_intake_check.build_report(Path.cwd())
    doc = Path("docs/codex/siem-export-adapter-external-response-intake.md").read_text(
        encoding="utf-8"
    )
    readme = Path("README.md").read_text(encoding="utf-8")
    makefile = Path("Makefile").read_text(encoding="utf-8")
    ...
    for blocked in [
        ...
        assert blocked in doc
    assert "make siem-export-adapter-external-response-intake-check" in readme
    assert "siem-export-adapter-external-response-intake-check:" in makefile
    assert "siem-export-adapter-external-response-intake-check" in release_check_body
    assert "siem-export-adapter-external-response-intake-check" in (
        release_guardrails.REQUIRED_RELEASE_CHECK_FRAGMENTS
    )
    assert "docs/codex/siem-export-adapter-external-response-intake.md" in docs_site
    assert "docs/codex/siem-export-adapter-external-response-intake.md" in (
        review_docs.REVIEW_DOCS
    )
    assert "SIEM Export Adapter External Response Intake" in review_index
...
def test_compliance_mapping_external_response_intake_is_wired() -> None:
    report = compliance_mapping_external_response_intake_check.build_report(Path.cwd())
    doc = Path("docs/codex/compliance-mapping-external-response-intake.md").read_text(
        encoding="utf-8"
    )
    makefile = Path("Makefile").read_text(encoding="utf-8")
    readme = Path("README.md").read_text(encoding="utf-8")
    ...
    packet_script = Path("scripts/compliance_mapping_disposition_packet.py").read_text(
        encoding="utf-8"
    )
    runway = Path("docs/codex/enterprise-readiness-runway.md").read_text(encoding="utf-8")
    gap_matrix = Path("docs/codex/enterprise-readiness-gap-matrix.md").read_text(
    ...
    assert report["new_power_classes_allowed"] is False
    assert report["public_security_product_positioning_allowed"] is False
    assert external_response_normalize.AREA_NAMESPACES["compliance-mapping"] == (
        "COMPLIANCE-MAPPING"
    )
    for phrase in [
    ...
    for phrase in [
        ...
        assert phrase not in doc
    assert "compliance-mapping-external-response-intake-check:" in makefile
    assert "compliance-mapping-external-response-intake-check" in release_check_body
    assert "compliance-mapping-external-response-intake-check" in release_guardrails
    assert "make compliance-mapping-external-response-intake-check" in readme
    assert "docs/codex/compliance-mapping-external-response-intake.md" in readme
    assert "docs/codex/compliance-mapping-external-response-intake.md" in docs_site
    assert (
        "docs/codex/compliance-mapping-external-response-intake.md"
        in review_docs.REVIEW_DOCS
    )
    assert "Compliance Mapping External Response Intake" in review_index
    assert "compliance-mapping-external-response-intake.md" in disposition_packet
    assert "compliance-mapping-external-response-intake.md" in architecture
    assert "compliance-mapping-external-response-intake.md" in packet_script
...
def test_operator_workbench_readiness_and_packet_are_wired(tmp_path: Path) -> None:
    ...
    assert demo_flow_report["tool_count"] == 24
...
def test_demo_evidence_closure_packet_and_readiness_are_wired(tmp_path: Path) -> None:
    ...
    assert readiness["tool_count"] == 24
...
def test_governed_artifact_transfer_lab_is_wired(tmp_path: Path) -> None:
    ...
    assert "tool count remains `24`" in doc
...
def test_hello_world_sandbox_demo_preimplementation_is_wired() -> None:
    ...
    assert report["runtime_changes_allowed"] is True
    assert report["new_power_classes_allowed"] is False
    assert report["write_capability_implemented"] is True
    ...
    assert "bounded sandbox artifact write is implemented" in roadmap
...
def test_trusted_host_promotion_negative_fixtures_are_wired() -> None:
    report = trusted_host_promotion_negative_fixtures_check.build_report(Path.cwd())
    fixtures = Path("docs/codex/trusted-host-promotion-negative-fixtures.md").read_text(
        encoding="utf-8"
    )
    readme = Path("README.md").read_text(encoding="utf-8")
    makefile = Path("Makefile").read_text(encoding="utf-8")
    ...
    assert report["erg_005_status"] == "blocked"
    assert report["prd_id"] == "PRD-TRUSTED-HOST-001"
    assert report["negative_case_count"] == 24
    assert report["negative_cases_rejected"] == 24
    assert report["decision_record_required"] is True
    assert report["implementation_approved"] is False
    assert report["runtime_changes_allowed"] is False
    assert report["trusted_host_promotion_allowed"] is False
    ...
    assert report["public_security_product_positioning_allowed"] is False
    for phrase in [
        "Status: design-only negative fixture contract for `ERG-005`",
        "TRUSTED-PROMOTION-NEG-001",
        "TRUSTED-PROMOTION-NEG-024",
    ...
    assert "trusted-host-promotion-negative-fixtures-check:" in makefile
    assert "trusted-host-promotion-negative-fixtures-check" in release_check_body
    assert "make trusted-host-promotion-negative-fixtures-check" in readme
    assert (
        "trusted-host-promotion-negative-fixtures-check"
        in release_guardrails.REQUIRED_RELEASE_CHECK_FRAGMENTS
    )
    assert "docs/codex/trusted-host-promotion-negative-fixtures.md" in review_docs.REVIEW_DOCS
    assert "docs/codex/trusted-host-promotion-negative-fixtures.md" in docs_site
    assert "Trusted-Host Promotion Negative Fixtures" in review_index
    assert "trusted-host-promotion-negative-fixtures.md" in enterprise
    assert "trusted-host-promotion-negative-fixtures.md" in gap_matrix
    assert "trusted-host-promotion-negative-fixtures.md" in decision_register
    assert "trusted-host-promotion-negative-fixtures.md" in intake
    assert "trusted-host-promotion-negative-fixtures.md" in state_machine


def test_trusted_host_promotion_zone_contract_is_wired() -> None:
    report = trusted_host_promotion_zone_contract_check.build_report(Path.cwd())
    contract = Path("docs/codex/trusted-host-promotion-zone-contract.md").read_text(
        encoding="utf-8"
    )
    readme = Path("README.md").read_text(encoding="utf-8")
    makefile = Path("Makefile").read_text(encoding="utf-8")
    ...
    assert report["erg_005_status"] == "blocked"
    assert report["prd_id"] == "PRD-TRUSTED-HOST-001"
    assert report["zone_prefixes"] == [
        "sandbox://",
        "host-staging://",
        "approved://",
        "evidence://",
    ]
    assert report["sample_label_count"] == 4
    assert report["unsafe_label_count"] == 4
    assert report["decision_record_required"] is True
    assert report["implementation_approved"] is False
    assert report["runtime_changes_allowed"] is False
    assert report["trusted_host_promotion_allowed"] is False
    ...
    assert "trusted-host-promotion-zone-contract-check:" in makefile
    assert "trusted-host-promotion-zone-contract-check" in release_check_body
    assert "make trusted-host-promotion-zone-contract-check" in readme
    assert (
        "trusted-host-promotion-zone-contract-check"
        in release_guardrails.REQUIRED_RELEASE_CHECK_FRAGMENTS
    )
    assert "docs/codex/trusted-host-promotion-zone-contract.md" in review_docs.REVIEW_DOCS
    assert "docs/codex/trusted-host-promotion-zone-contract.md" in docs_site
    assert "Trusted-Host Promotion Zone Contract" in review_index
    assert "trusted-host-promotion-zone-contract.md" in enterprise
    assert "trusted-host-promotion-zone-contract.md" in gap_matrix
    assert "trusted-host-promotion-zone-contract.md" in decision_register
    assert "trusted-host-promotion-zone-contract.md" in intake
    assert "trusted-host-promotion-zone-contract.md" in state_machine
    assert "trusted-host-promotion-zone-contract.md" in negative_fixtures


def test_trusted_host_promotion_implementation_plan_is_wired() -> None:
    report = trusted_host_promotion_implementation_plan_check.build_report(Path.cwd())
    plan = Path("docs/codex/trusted-host-promotion-implementation-plan.md").read_text(
        encoding="utf-8"
    )
    readme = Path("README.md").read_text(encoding="utf-8")
    makefile = Path("Makefile").read_text(encoding="utf-8")
    ...
    for phrase in [
        ...
        "sandbox://artifact -> host-staging://artifact -> approved://artifact",
        "one artifact, one approval, one promotion attempt",
    ...
    assert "trusted-host-promotion-implementation-plan-check:" in makefile
    assert "trusted-host-promotion-implementation-plan-check" in release_check_body
    assert "make trusted-host-promotion-implementation-plan-check" in readme
    assert (
    ...
    assert "docs/codex/trusted-host-promotion-implementation-plan.md" in docs_site
    assert "Trusted-Host Promotion Implementation Plan" in review_index
    assert "trusted-host-promotion-implementation-plan.md" in enterprise
    assert "trusted-host-promotion-implementation-plan.md" in gap_matrix
    assert "trusted-host-promotion-implementation-plan.md" in decision_register
    assert "trusted-host-promotion-implementation-plan.md" in intake
...
def test_trusted_host_promotion_external_review_bundle_is_wired(
    tmp_path: Path,
) -> None:
    report = trusted_host_promotion_external_review_bundle.build_check_report(Path.cwd())
    output_dir = tmp_path / "trusted-host-promotion-external-review"
    trusted_host_promotion_external_review_bundle.build_bundle(
        repo_root=Path.cwd(),
        output_dir=output_dir,
        allow_dirty=True,
        run_commands=False,
    ...
    expected = {
        ...
        "trusted-host-promotion-external-review-artifact-hashes.json",
    }
    generated = {path.name for path in output_dir.iterdir()}
    hashes = json.loads(
        (
            output_dir / "trusted-host-promotion-external-review-artifact-hashes.json"
        ).read_text(encoding="utf-8")
    )
    index = (
        output_dir / "00_TRUSTED_HOST_PROMOTION_EXTERNAL_REVIEW_INDEX.md"
    ).read_text(encoding="utf-8")
    prompt = (
        output_dir / "01_TRUSTED_HOST_PROMOTION_EXTERNAL_REVIEW_PROMPT.md"
    ).read_text(encoding="utf-8")
    source_packet = (
        output_dir / "02_TRUSTED_HOST_PROMOTION_SOURCE_PACKET.md"
    ).read_text(encoding="utf-8")
    disposition_packet = (
        output_dir / "03_TRUSTED_HOST_PROMOTION_DISPOSITION_PACKET.md"
    ).read_text(encoding="utf-8")
    contracts = (
        output_dir / "04_TRUSTED_HOST_PROMOTION_CONTRACTS.md"
    ).read_text(encoding="utf-8")
    negatives = (
        output_dir / "05_TRUSTED_HOST_PROMOTION_FIXTURES_NEGATIVES.md"
    ).read_text(encoding="utf-8")
    response = (
        output_dir / "06_TRUSTED_HOST_PROMOTION_RESPONSE_CLOSURE_DRY_RUN.md"
    ).read_text(encoding="utf-8")
    reproduction = (
        output_dir / "07_TRUSTED_HOST_PROMOTION_REPRODUCTION_QUEUE_STATUS.md"
    ).read_text(encoding="utf-8")
    evidence = (
        output_dir / "08_TRUSTED_HOST_PROMOTION_COMMAND_EVIDENCE.md"
    ).read_text(encoding="utf-8")
    readme = Path("README.md").read_text(encoding="utf-8")
    makefile = Path("Makefile").read_text(encoding="utf-8")
    docs_site = Path("scripts/build_docs_site.py").read_text(encoding="utf-8")
    ...
    assert "TRUSTED_HOST_PROMOTION_DISPOSITION_PROMPT" in disposition_packet
    assert "trusted-host-promotion-state-machine.md" in contracts
    assert "trusted-host-promotion-zone-contract.md" in contracts
    assert "trusted-host-promotion-negative-fixtures.md" in negatives
    assert "sandbox-artifact-observed-demo.md" in negatives
    assert "trusted-host-promotion-external-response-intake.md" in response
    assert "trusted-host-promotion-disposition-closure-gate.md" in response
    assert "trusted-host-promotion-response-dry-run.md" in response
    assert "enterprise-external-review-queue.md" in reproduction
    assert "trusted-host-promotion-source-review.md" in reproduction
    assert "trusted-host-promotion-disposition-packet.md" in reproduction
    for flag in [
        '"runtime_changes_allowed": false',
        '"trusted_host_promotion_allowed": false',
        '"direct_host_writes_allowed": false',
        ...
        assert flag in evidence
    assert "make trusted-host-promotion-source-review-packet-check" in evidence
    assert "make trusted-host-promotion-external-review-bundle" in readme
    assert "trusted-host-promotion-external-review-bundle:" in makefile
    assert "trusted-host-promotion-external-review-bundle-check:" in makefile
    assert "trusted-host-promotion-external-review-bundle-check" in release_check_body
    assert "$(MAKE) trusted-host-promotion-external-review-bundle" in review_candidate_body
    assert "trusted-host-promotion-external-review-bundle-check" in (
        release_guardrails.REQUIRED_RELEASE_CHECK_FRAGMENTS
    )
    assert "$(MAKE) trusted-host-promotion-external-review-bundle" in (
        release_guardrails.REQUIRED_REVIEW_CANDIDATE_STEPS
    )
    assert (
    ...
    assert "docs/codex/trusted-host-promotion-external-review-bundle.md" in docs_site
    assert "Trusted-Host Promotion External Review Bundle" in review_index
    assert "trusted-host-promotion-external-review-bundle.md" in runway
    assert "trusted-host-promotion-external-review-bundle.md" in gap_matrix
    assert "trusted-host-promotion-external-review-bundle.md" in queue
...
def test_trusted_host_promotion_disposition_closure_gate_is_wired() -> None:
    report = trusted_host_promotion_disposition_closure_check.build_report(Path.cwd())
    doc = Path("docs/codex/trusted-host-promotion-disposition-closure-gate.md").read_text(
        encoding="utf-8"
    )
    readme = Path("README.md").read_text(encoding="utf-8")
    makefile = Path("Makefile").read_text(encoding="utf-8")
    ...
    intake = Path("docs/codex/trusted-host-promotion-external-response-intake.md").read_text(
        encoding="utf-8"
    )
    disposition_packet = Path("docs/codex/trusted-host-promotion-disposition-packet.md").read_text(
        encoding="utf-8"
    )
    readiness = Path("docs/codex/enterprise-sandbox-control-plane-readiness.md").read_text(
        encoding="utf-8"
    ...
    for forbidden in [
        ...
        assert forbidden not in doc
    assert "make trusted-host-promotion-disposition-closure-check" in readme
    assert "trusted-host-promotion-disposition-closure-check:" in makefile
    assert (
        "trusted-host-promotion-disposition-closure-check" in release_check_body
        or "release-check: trusted-host-promotion-disposition-closure-check" in makefile
    )
    assert "trusted-host-promotion-disposition-closure-check" in (
        release_guardrails.REQUIRED_RELEASE_CHECK_FRAGMENTS
    )
    assert "docs/codex/trusted-host-promotion-disposition-closure-gate.md" in docs_site
    assert (
        "docs/codex/trusted-host-promotion-disposition-closure-gate.md"
        in review_docs.REVIEW_DOCS
    )
    assert "Trusted-Host Promotion Disposition Closure Gate" in review_index
    assert "trusted-host-promotion-disposition-closure-gate.md" in enterprise
    assert "trusted-host-promotion-disposition-closure-gate.md" in gap_matrix
    assert "trusted-host-promotion-disposition-closure-gate.md" in queue
    assert "trusted-host-promotion-disposition-closure-gate.md" in decision_register
...
def test_hello_world_sandbox_demo_packet_builds_from_fixture(tmp_path: Path) -> None:
    ...
    assert manifest["tool_count"] == 24
    assert manifest["runtime_write_capability_implemented"] is True
    assert manifest["governed_tool_calls_performed"] is False
...
def test_sandbox_artifact_write_text_preimplementation_is_historical() -> None:
    ...
    assert report["scope"] == "historical_preimplementation_planning"
    assert report["implementation_allowed"] is True
    assert report["runtime_changes_allowed"] is True
    assert report["write_capability_implemented"] is True
    assert report["tool_count"] == 24
    assert report["manifest_absent"] is False
    assert report["superseded_by_runtime_implementation"] is True
    ...
    assert "sandbox-artifact-write-text-preimplementation-check" not in release_check_body
...
def test_sandbox_artifact_write_text_implementation_gate_is_wired() -> None:
    ...
    assert report["implementation_status"] == "bounded_local_preview_runtime"
    assert report["runtime_implemented"] is True
    assert report["runtime_changes_allowed_now"] is True
    assert report["tool_count"] == 24
    assert report["manifest_present"] is True
    assert "Status: runtime implementation approved for bounded local-preview use." in decision
    assert "Runtime implementation is present" in decision
