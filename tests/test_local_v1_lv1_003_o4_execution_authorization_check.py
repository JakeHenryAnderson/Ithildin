...
def test_prepared_gate_018_preserves_prior_consumed_history() -> None:
    contract = _contract()
    ...
    assert contract["attempt_018_execution_authorized"] is True
...
def test_attempt_016_consumed_contract_is_preserved_under_attempt_018_preparation() -> None:
    contract = _contract()
    failures: list[str] = []
...
def test_attempt_017_consumed_closure_cannot_authorize_attempt_018(
    tmp_path: Path,
    monkeypatch: pytest.MonkeyPatch,
) -> None:
...
def test_attempt_018_exact_six_path_candidate_and_annotated_tag_binding(
    tmp_path: Path,
) -> None:
    repo, commit, tree = _attempt_018_candidate_repository(
        tmp_path,
        tag_candidate=True,
    )
    failures: list[str] = []
    ...
    checkout = gate._validate_execution_checkout(  # noqa: SLF001
        ...
        reviewed_commit=gate.MISSION_LIVENESS_TERMINAL_REASON_COMMIT,
        runtime_paths=gate.ATTEMPT_018_RUNTIME_PATHS,
        control_paths=gate.ATTEMPT_018_CONTROL_PATH_ALLOWLIST,
    )
    gate._validate_attempt_018_review_binding(  # noqa: SLF001
        repo,
        candidate_commit=commit,
        candidate_tree=tree,
        failures=failures,
    ...
    assert failures == []
    assert checkout == (commit, tree)
    assert len(gate.ATTEMPT_018_CONTROL_PATH_ALLOWLIST) == 6


def test_attempt_018_pretag_candidate_fails_closed(
    tmp_path: Path,
) -> None:
    repo, commit, tree = _attempt_018_candidate_repository(
        tmp_path,
    ...
    assert failures == ["O4 Attempt 018 review tag is missing or not annotated"]


def test_attempt_018_tagged_candidate_has_one_shot_authority(
    tmp_path: Path,
    monkeypatch: pytest.MonkeyPatch,
) -> None:
    repo, commit, tree = _attempt_018_candidate_repository(
    ...
    assert report["attempt_id"] == gate.ATTEMPT_018_ID
    ...
    assert report["next_action"] == (
        "execute_attempt_018_once_and_record_immediate_consumed_disposition"
    )
    assert report["release_allowed"] is False
    assert report["uat_complete"] is False


def test_attempt_018_pretag_candidate_effective_authority_is_closed(
    tmp_path: Path,
    monkeypatch: pytest.MonkeyPatch,
) -> None:
    repo, _, _ = _attempt_018_candidate_repository(
        tmp_path,
        tag_candidate=False,
    )
    _skip_private_evidence(monkeypatch)
    ...
    assert report["valid"] is False
    assert report["attempt_id"] == gate.ATTEMPT_018_ID
    assert report["attempted_candidate_commit"] == report["execution_checkout_commit"]
    assert report["attempted_candidate_tree"] == report["execution_checkout_tree"]
    assert report["failures"] == [
        "O4 Attempt 018 review tag is missing or not annotated"
    ]
    assert report["execution_attempt_budget"] == 0
    assert report["live_execution_authorized"] is False
...
def test_attempt_018_lightweight_tag_fails_closed(
    tmp_path: Path,
) -> None:
    repo, commit, tree = _attempt_018_candidate_repository(
        tmp_path,
    ...
    _run_git(repo, "tag", gate.ATTEMPT_018_REVIEW_TAG, commit)
    failures: list[str] = []
...
def test_attempt_018_prepared_contract_and_liveness_review_are_exact() -> None:
    contract = _contract()
    failures: list[str] = []
...
@pytest.mark.parametrize(
    ...
    [
        ...
        (
            ...
            "authority is not the exact bounded Attempt 014 posture",
        ),
        (
            lambda value: value["authority"].__setitem__(  # type: ignore[union-attr]
...
def test_wiring_rejects_stale_readme_attempt_guidance(tmp_path: Path) -> None:
    ...
    readme = Path("README.md").read_text(encoding="utf-8").replace(
        "prepared gate-protected one-shot Attempt 018 entrypoint",
    ...
    assert any(
        "README is missing current O4 Attempt 018 guidance" in failure for failure in failures
    )
