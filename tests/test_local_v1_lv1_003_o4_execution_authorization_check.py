...
def _attempt_008_child_repository(tmp_path: Path) -> tuple[Path, str, str]:
    repo = tmp_path / "candidate"
    subprocess.run(
        ["git", "clone", "-q", str(Path.cwd()), str(repo)],
        check=True,
    )
    _run_git(repo, "checkout", "--detach", gate.ATTEMPT_008_CANDIDATE_COMMIT)
    for relative in gate.ATTEMPT_008_CLOSURE_CONTROL_PATH_ALLOWLIST:
        destination = repo / relative
        destination.parent.mkdir(parents=True, exist_ok=True)
        shutil.copy2(relative, destination)
    _run_git(repo, "add", "--", *gate.ATTEMPT_008_CLOSURE_CONTROL_PATH_ALLOWLIST)
    _run_git(
        repo,
        "-c",
        ...
        "-q",
        "-m",
        "test: exact O4 Attempt 008 closure child",
    )
    shutil.copytree(
...
def test_attempt_008_exact_closure_child_keeps_all_authority_closed(
    tmp_path: Path,
) -> None:
    repo, execution_commit, execution_tree = _attempt_008_child_repository(tmp_path)
    ...
    assert report["attempt_id"] == gate.ATTEMPT_008_ID
    assert report["attempted_candidate_commit"] == gate.ATTEMPT_008_CANDIDATE_COMMIT
    assert report["attempted_candidate_tree"] == gate.ATTEMPT_008_CANDIDATE_TREE
    ...
    assert contract["candidate_parent_commit"] == gate.ATTEMPT_008_CANDIDATE_COMMIT
    assert contract["candidate_parent_tree"] == gate.ATTEMPT_008_CANDIDATE_TREE
    assert (
...
def test_additional_retained_attempt_run_refuses_attempt_004(
    ...
    repo, _, _ = _attempt_008_child_repository(tmp_path)
...
def test_bound_review_or_disposition_digest_drift_is_rejected(
    ...
    for relative in (
        ...
        destination.parent.mkdir(parents=True, exist_ok=True)
        shutil.copy2(relative, destination)
    (tmp_path / gate.IMAGE_READABILITY_REPAIR_REVIEW).write_text(
        "altered\n",
        encoding="utf-8",
    )
    failures: list[str] = []
    ...
    assert any("image-readability repair exact review digest" in value for value in failures)
...
def test_execution_checkout_rejects_sibling_of_reviewed_parent(
    ...
    _run_git(
        ...
        "checkout",
        "--detach",
        f"{gate.ATTEMPT_008_CANDIDATE_COMMIT}^",
    )
    for relative in gate.ATTEMPT_008_CLOSURE_CONTROL_PATH_ALLOWLIST:
        destination = repo / relative
        destination.parent.mkdir(parents=True, exist_ok=True)
...
def test_wiring_rejects_stale_readme_attempt_guidance(tmp_path: Path) -> None:
    ...
    assert any(
        "README is missing current O4 Attempt 008 guidance" in failure for failure in failures
    )
