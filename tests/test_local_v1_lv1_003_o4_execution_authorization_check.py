...
def _attempt_008_child_repository(tmp_path: Path) -> tuple[Path, str, str]:
    ...
    _run_git(repo, "checkout", "--detach", gate.IMAGE_READABILITY_REPAIR_COMMIT)
    for relative in gate.ATTEMPT_008_CONTROL_PATH_ALLOWLIST:
        destination = repo / relative
        destination.parent.mkdir(parents=True, exist_ok=True)
        shutil.copy2(relative, destination)
    _run_git(repo, "add", "--", *gate.ATTEMPT_008_CONTROL_PATH_ALLOWLIST)
    _run_git(
        repo,
        "-c",
        ...
        "-q",
        "-m",
        "test: exact O4 Attempt 008 authorization child",
    )
    shutil.copytree(
...
def test_attempt_008_exact_child_authorizes_one_supervised_invocation(
    tmp_path: Path,
) -> None:
    ...
    assert report["record_status"] == "ATTEMPT_008_EXACT_CHILD_ONE_SHOT_EXECUTION_AUTHORIZED"
    ...
    assert contract["candidate_parent_commit"] == gate.IMAGE_READABILITY_REPAIR_COMMIT
    assert contract["candidate_parent_tree"] == gate.IMAGE_READABILITY_REPAIR_TREE
    assert (
...
def test_execution_checkout_rejects_sibling_of_reviewed_parent(
    ...
    _run_git(
        ...
        "checkout",
        "--detach",
        f"{gate.IMAGE_READABILITY_REPAIR_COMMIT}^",
    )
    for relative in gate.ATTEMPT_008_CONTROL_PATH_ALLOWLIST:
        destination = repo / relative
        destination.parent.mkdir(parents=True, exist_ok=True)
