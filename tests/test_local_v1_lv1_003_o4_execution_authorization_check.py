...
def _closure_child_repository(tmp_path: Path) -> tuple[Path, str, str]:
    ...
    _run_git(repo, "checkout", "--detach", gate.ATTEMPT_007_CANDIDATE_COMMIT)
    for relative in gate.ATTEMPT_007_CLOSURE_CONTROL_PATH_ALLOWLIST:
        destination = repo / relative
        destination.parent.mkdir(parents=True, exist_ok=True)
        shutil.copy2(relative, destination)
    _run_git(repo, "add", "--", *gate.ATTEMPT_007_CLOSURE_CONTROL_PATH_ALLOWLIST)
    _run_git(
        repo,
        "-c",
        ...
        "-q",
        "-m",
        "test: exact O4 Attempt 007 closure child",
    )
    shutil.copytree(
...
def test_attempt_007_exact_closure_child_keeps_all_authority_closed(
    tmp_path: Path,
) -> None:
    ...
    assert report["record_status"] == (
        "ATTEMPT_007_CONSUMED_PRELAUNCH_IMAGE_READABILITY_INVESTIGATION_REQUIRED_NO_LIVE_AUTHORITY"
    )
    ...
    assert report["attempt_id"] == gate.ATTEMPT_007_ID
    assert report["attempted_candidate_commit"] == gate.ATTEMPT_007_CANDIDATE_COMMIT
    assert report["attempted_candidate_tree"] == gate.ATTEMPT_007_CANDIDATE_TREE
    ...
    assert contract["candidate_parent_commit"] == gate.ATTEMPT_007_CANDIDATE_COMMIT
    assert contract["candidate_parent_tree"] == gate.ATTEMPT_007_CANDIDATE_TREE
    assert (
...
def test_execution_checkout_rejects_sibling_of_reviewed_parent(
    ...
    _run_git(
        ...
        "checkout",
        "--detach",
        f"{gate.ATTEMPT_007_CANDIDATE_COMMIT}^",
    )
    for relative in gate.ATTEMPT_007_CLOSURE_CONTROL_PATH_ALLOWLIST:
        destination = repo / relative
        destination.parent.mkdir(parents=True, exist_ok=True)
...
def test_wiring_rejects_stale_readme_attempt_guidance(tmp_path: Path) -> None:
    ...
    assert any(
        "README is missing current O4 Attempt 007 guidance" in failure for failure in failures
    )
