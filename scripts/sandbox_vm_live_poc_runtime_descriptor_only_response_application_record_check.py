"""Validate ERG-004 descriptor-only response-application record wiring."""
...
REQUIRED_PHRASES = [
    (
        "Status: process-only response-application record for the `ERG-004` "
        "descriptor-only source review."
    ),
...
def build_report(repo_root: Path) -> dict[str, Any]:
    ...
    if not doc:
        failures.append("descriptor-only response application record doc is missing")
    else:
    ...
    if intake_report.get("valid") is not True:
        failures.append("descriptor-only response intake is not valid")
    if intake_report.get("runtime_changes_allowed") is not False:
        failures.append("intake unexpectedly allows runtime changes")

    target = "sandbox-vm-live-poc-runtime-descriptor-only-response-application-record-check"
...
def _report(valid: bool, failures: list[str], doc_key: str) -> dict[str, Any]:
    return {
        ...
        "allowed_future_status": "descriptor_only_local_preview_disposition_ready",
...
def render_report(report: dict[str, Any]) -> str:
    lines = [
        "Ithildin ERG-004 descriptor-only response application record check",
