"""Validate the fail-closed trusted-host promotion disposition closure gate."""
...
DOC_REL = "docs/codex/trusted-host-promotion-disposition-closure-gate.md"
DOC_NAME = "trusted-host-promotion-disposition-closure-gate.md"
...
def build_report(repo_root: Path) -> dict[str, Any]:
    ...
    intake = _read(repo_root / "docs/codex/trusted-host-promotion-external-response-intake.md")
    ...
    if not doc_path.exists():
        failures.append("trusted-host promotion disposition closure gate doc is missing")
        text = ""
    else:
    ...
    for linked_text, source_name in [
        ...
        (disposition_packet, "trusted-host disposition packet"),
    ...
    if DOC_REL not in review_docs.REVIEW_DOCS:
        failures.append("review_docs.REVIEW_DOCS is missing trusted-host closure gate doc")
    if "trusted-host-promotion-disposition-closure-check:" not in makefile:
        failures.append("Make target is missing: trusted-host-promotion-disposition-closure-check")
    release_check_additive = "release-check: trusted-host-promotion-disposition-closure-check"
    if (
        "trusted-host-promotion-disposition-closure-check" not in release_check_body
        and release_check_additive not in makefile
    ):
        failures.append(
            "trusted-host-promotion-disposition-closure-check missing from release-check"
        )
    if "trusted-host-promotion-disposition-closure-check" not in release_guardrails:
        failures.append("release guardrails do not require trusted-host disposition closure-check")
...
def render_report(report: dict[str, Any]) -> str:
    lines = [
        "Ithildin trusted-host promotion disposition closure check",
