"""Build the project.ci.summary design-review packet."""
...
DEFAULT_OUTPUT_DIR = Path("var/review-packets/v3/project-ci-summary-design-review")
...
DOCS = {
    ...
    "03_PROJECT_CI_SUMMARY_SELECTION.md": Path("docs/codex/v3-project-ci-summary-selection.md"),
    "04_PROJECT_CI_SUMMARY_PROPOSAL.md": Path(
        "docs/codex/capability-proposals/project-ci-summary.md"
    ),
    "05_PROJECT_CI_SUMMARY_IMPLEMENTATION_PLAN.md": Path(
        "docs/codex/capability-implementation-plans/project-ci-summary.md"
    ),
}


class ProjectCiSummaryDesignReviewPacketError(RuntimeError):
...
def main() -> int:
    ...
    except ProjectCiSummaryDesignReviewPacketError as exc:
        print(f"project.ci.summary design-review packet failed: {exc}", file=sys.stderr)
        return 1

    print(f"Built project.ci.summary design-review packet at {output_dir}")
    return 0
...
def build_packet(*, repo_root: Path, output_dir: Path, allow_dirty: bool = False) -> Path:
    ...
    if dirty and not allow_dirty:
        raise ProjectCiSummaryDesignReviewPacketError(
            "working tree is dirty; commit before design-review handoff"
        )

    proposal = project_ci_summary_proposal_check.build_report(repo_root)
    plan = project_ci_summary_implementation_plan_check.build_report(repo_root)
    ...
    if failures:
        raise ProjectCiSummaryDesignReviewPacketError("; ".join(failures))
    ...
    files = {
        "00_PROJECT_CI_SUMMARY_DESIGN_REVIEW_INDEX.md": _index(context),
        "01_PROJECT_CI_SUMMARY_DESIGN_REVIEW_PROMPT.md": _prompt(context),
    ...
    _write_json(
        output_dir / "project-ci-summary-design-review-artifact-hashes.json",
        _hashes(output_dir),
    )
    return output_dir
...
def _index(context: dict[str, Any]) -> str:
    return f"""# project.ci.summary Design Review Packet
...
1. `00_PROJECT_CI_SUMMARY_DESIGN_REVIEW_INDEX.md`
2. `01_PROJECT_CI_SUMMARY_DESIGN_REVIEW_PROMPT.md`
3. `02_NEXT_CAPABILITY_READINESS.md`
4. `03_PROJECT_CI_SUMMARY_SELECTION.md`
5. `04_PROJECT_CI_SUMMARY_PROPOSAL.md`
6. `05_PROJECT_CI_SUMMARY_IMPLEMENTATION_PLAN.md`
7. `06_GATE_AND_RISK_EVIDENCE.md`
8. `07_REVIEW_INTAKE_AND_NEXT_STEPS.md`
9. `project-ci-summary-design-review-artifact-hashes.json`

## What This Packet Does Not Prove

This packet does not add or approve a manifest, executor, policy rule, MCP exposure, API behavior,
UI behavior, runtime behavior, workflow names, raw paths, file contents, command/script values,
environment names or values, CI execution, shell access, registry/network access, deployment
readiness, compliance claims, or future governed tool powers.
"""


def _prompt(context: dict[str, Any]) -> str:
    return f"""# project.ci.summary Design Review Prompt
...
Reviewed commit: `{context["commit"]}`
Capability: `project.ci.summary`
Finding namespace: `EXT-DESIGN-PCI-###`

Please review:

- whether the proposal stays design-only and avoids manifest/executor/policy/MCP/runtime changes;
- whether count-only CI posture metadata is useful enough without workflow names, raw paths,
  file contents, command/script values, environment names or values, or CI execution;
- whether the future traversal and provider/category allowlists preserve existing workspace and
  path safety;
...
Use finding IDs `EXT-DESIGN-PCI-###` for actionable findings.
"""


def _gate_evidence(context: dict[str, Any]) -> str:
    return f"""# project.ci.summary Gate And Risk Evidence
...
def _hashes(output_dir: Path) -> list[dict[str, Any]]:
    ...
    for path in sorted(output_dir.iterdir()):
        if path.name == "project-ci-summary-design-review-artifact-hashes.json":
            continue
