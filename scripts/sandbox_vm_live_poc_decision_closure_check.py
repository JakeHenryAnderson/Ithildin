"""Validate the fail-closed sandbox/VM live POC decision closure gate."""
...
DOC_REL = "docs/codex/sandbox-vm-live-poc-decision-closure-gate.md"
DOC_NAME = "sandbox-vm-live-poc-decision-closure-gate.md"
NORMALIZED_RESPONSE_REL = "var/review-runs/sandbox-vm-live-poc/normalized-response.json"
EXPECTED_AREA = "sandbox-vm-live-poc"
EXPECTED_NAMESPACE = "EXT-LIVE-POC-###"
...
def build_report(repo_root: Path) -> dict[str, Any]:
    ...
    intake = _read(repo_root / "docs/codex/sandbox-vm-live-poc-external-response-intake.md")
    ...
    if not doc_path.exists():
        failures.append("sandbox/VM live POC decision closure gate doc is missing")
        text = ""
    else:
    ...
    if DOC_REL not in review_docs.REVIEW_DOCS:
        failures.append("review_docs.REVIEW_DOCS is missing live POC closure gate doc")
    if "sandbox-vm-live-poc-decision-closure-check:" not in makefile:
        failures.append("Make target is missing: sandbox-vm-live-poc-decision-closure-check")
    release_check_additive = "release-check: sandbox-vm-live-poc-decision-closure-check"
    if (
        "sandbox-vm-live-poc-decision-closure-check" not in release_check_body
        and release_check_additive not in makefile
    ):
        failures.append("sandbox-vm-live-poc-decision-closure-check missing from release-check")
    if "sandbox-vm-live-poc-decision-closure-check" not in release_guardrails:
        failures.append("release guardrails do not require live POC decision closure-check")
    ...
    return {
        ...
        "erg_003_favorable_disposition": response_report["erg_003_favorable_disposition"],
        "decision_outcome": response_report["decision_outcome"],
        "erg_004_status": "blocked" if not closure_ready else "ready_for_decision_record",
...
def _validate_normalized_response(path: Path) -> dict[str, Any]:
    if not path.exists():
        return {
            ...
            "reason": "normalized response is absent; ERG-004 remains blocked",
        }
    failures: list[str] = []
    try:
    ...
    if payload.get("area") != EXPECTED_AREA:
        failures.append("normalized response area is not sandbox-vm-live-poc")
    ...
    erg_003_favorable = payload.get("erg_003_favorable_disposition") is True
    if not erg_003_favorable:
        failures.append("normalized response lacks favorable ERG-003 disposition evidence")
    decision_outcome = payload.get("decision_outcome")
    if decision_outcome != "approve_limited_operator_managed_poc_planning":
        failures.append("normalized response decision_outcome does not permit planning review")
    ...
    for finding in findings:
        ...
        if not finding_id.startswith("EXT-LIVE-POC-"):
    ...
    return {
        ...
        "erg_003_favorable_disposition": erg_003_favorable,
...
def render_report(report: dict[str, Any]) -> str:
    lines = [
        "Ithildin sandbox/VM live POC decision closure check",
        ...
        "erg_003_favorable_disposition: "
        f"{str(report['erg_003_favorable_disposition']).lower()}",
