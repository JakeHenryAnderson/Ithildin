"""Build a reviewer-response kit for ERG-002 Mission Control display/import planning."""
...
DEFAULT_OUTPUT_DIR = Path("var/review-packets/v3/mission-control-display-response-kit")
HASH_MANIFEST = "mission-control-display-response-kit-artifact-hashes.json"
PROJECT_MARKERS = [
    Path("pyproject.toml"),
    Path("Makefile"),
    Path("apps/api"),
...
class MissionControlDisplayResponseKitError(RuntimeError):
...
def main() -> int:
    ...
    except MissionControlDisplayResponseKitError as exc:
        print(f"Mission Control display response kit failed: {exc}", file=sys.stderr)
        return 1

    print(f"Built Mission Control display response kit at {output_dir}")
    return 0
...
def build_check_report(repo_root: Path) -> dict[str, Any]:
    ...
    mission_control_readiness = _read(
    ...
    with tempfile.TemporaryDirectory() as tmp:
        output_dir = Path(tmp) / "mission-control-display-response-kit"
        try:
            build_kit(
                repo_root=repo_root,
            ...
            contents = {name: (output_dir / name).read_text(encoding="utf-8") for name in ARTIFACTS}
    ...
    index = contents.get("00_MISSION_CONTROL_DISPLAY_RESPONSE_KIT_INDEX.md", "")
    guide = contents.get("01_MISSION_CONTROL_DISPLAY_RESPONSE_INTAKE_GUIDE.md", "")
    examples = contents.get("02_MISSION_CONTROL_DISPLAY_NORMALIZED_RESPONSE_EXAMPLES.md", "")
    commands = contents.get("03_MISSION_CONTROL_DISPLAY_CLOSURE_TRIAGE_COMMANDS.md", "")
    boundary = contents.get("04_MISSION_CONTROL_DISPLAY_QUEUE_AND_BOUNDARY_STATUS.md", "")
    evidence = contents.get("05_MISSION_CONTROL_DISPLAY_RESPONSE_KIT_EVIDENCE.md", "")
    ...
    for phrase in [
        ...
        "Only a later committed triage update may move `ERG-002`",
    ]:
    ...
    target = "mission-control-display-response-kit"
    ...
    if check_target not in release_check_body:
        additive_release_check = f"release-check: {check_target}"
        if additive_release_check not in makefile:
    ...
    doc_rel = "docs/codex/mission-control-display-response-kit.md"
    ...
    if "Mission Control Display Response Kit" not in review_index:
    ...
    for text, source in [
        ...
        (mission_control_readiness, "Mission Control integration readiness packet"),
    ]:
        if "mission-control-display-response-kit" not in text:
...
def build_kit(
    *,
    ...
    if dirty and not allow_dirty:
        raise MissionControlDisplayResponseKitError(
    ...
    files = {
        "00_MISSION_CONTROL_DISPLAY_RESPONSE_KIT_INDEX.md": _index(commit, dirty),
        "01_MISSION_CONTROL_DISPLAY_RESPONSE_INTAKE_GUIDE.md": _intake_guide(),
        "02_MISSION_CONTROL_DISPLAY_NORMALIZED_RESPONSE_EXAMPLES.md": _examples(),
        "03_MISSION_CONTROL_DISPLAY_CLOSURE_TRIAGE_COMMANDS.md": _commands(),
        "04_MISSION_CONTROL_DISPLAY_QUEUE_AND_BOUNDARY_STATUS.md": _docs_bundle(
        ...
        "05_MISSION_CONTROL_DISPLAY_RESPONSE_KIT_EVIDENCE.md": _command_evidence(command_reports),
    }
...
def _index(commit: str, dirty: bool) -> str:
    boundary = (
        "This response kit does not prove external review happened, does not close `ERG-002`, "
        "does not approve Mission Control runtime importer behavior, does not approve Mission "
        "Control execution authority, Mission Control policy authority, Mission Control approval "
        "authority, Mission Control audit authority, API callbacks, polling or mutating Ithildin "
        "APIs, local model "
        "invocation, VM/container lifecycle management, sandbox orchestration, trusted-host "
        "promotion, network expansion, new governed tool powers, production identity, runtime "
        "Postgres, hosted telemetry, remote MCP, SIEM delivery, compliance automation, or "
        "public/security-product positioning."
    )
    return f"""# Mission Control Display Response Kit

Status: response-intake kit for `ERG-002` after external/source review.
...
1. `01_MISSION_CONTROL_DISPLAY_RESPONSE_INTAKE_GUIDE.md`
2. `02_MISSION_CONTROL_DISPLAY_NORMALIZED_RESPONSE_EXAMPLES.md`
3. `03_MISSION_CONTROL_DISPLAY_CLOSURE_TRIAGE_COMMANDS.md`
4. `04_MISSION_CONTROL_DISPLAY_QUEUE_AND_BOUNDARY_STATUS.md`
5. `05_MISSION_CONTROL_DISPLAY_RESPONSE_KIT_EVIDENCE.md`
6. `mission-control-display-response-kit-artifact-hashes.json`
...
Only a later committed triage update may move `ERG-002`, and only after the closure gate reports
`closure_ready: true`. A response may support a design-only Mission Control display/import
decision record only. It must not approve a runtime importer, Mission Control execution authority,
Mission Control policy authority, Mission Control approval authority, Mission Control audit
authority, API callbacks, polling or mutating Ithildin APIs, live VM/container control, local model
invocation, sandbox orchestration, trusted-host promotion, or API/MCP profile loading.
"""
        ...
        "findings": [
            {
                ...
                "severity": "high",
                "area": "mission-control-display",
                "affected_files_functions": "scripts/mission_control_display.py",
...
update following `mission-control-display-decision-intake.md`, then run:
...
external review directly, keep `ERG-002` as `planning_only` and keep Mission Control runtime
importer behavior blocked.
"""
...
def _build_command_reports(repo_root: Path, *, run_commands: bool) -> dict[str, Any]:
    reports: dict[str, Any] = {
        ...
        "disposition_closure_check": (
            mission_control_display_disposition_closure_check.build_report(repo_root)
        ),
...
def render_check_report(report: dict[str, Any]) -> str:
    lines = [
        "Ithildin Mission Control display response kit check",
