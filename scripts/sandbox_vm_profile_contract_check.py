"""Validate the design-only sandbox/VM profile contract."""
...
DOC = ROOT / "docs/codex/sandbox-vm-profile-contract.md"

REQUIRED_PHRASES = [
    "Status: design-only profile contract",
    ...
    "warning_state",
    "working_root_label",
    "host_staging_root_label",
    "approved_output_root_label",
    "mount_posture",
    "network_posture",
    "cleanup_posture",
    "evidence_output_label",
    "promotion_status",
...
def build_report(repo_root: Path) -> dict[str, Any]:
    ...
    if not DOC.exists():
        failures.append("sandbox/VM profile contract is missing")
        text = ""
    else:
        text = DOC.read_text(encoding="utf-8")
    for phrase in REQUIRED_PHRASES:
        if phrase not in text:
            failures.append(f"sandbox/VM profile contract is missing phrase: {phrase}")
    for claim in FORBIDDEN_CLAIMS:
        if claim in text:
            failures.append(f"sandbox/VM profile contract contains forbidden overclaim: {claim}")
    if rel_path not in review_docs.REVIEW_DOCS:
        failures.append("sandbox/VM profile contract is missing from review docs")
    if rel_path not in docs_site:
        failures.append("sandbox/VM profile contract is missing from docs-site inputs")
    if "make sandbox-vm-profile-contract-check" not in readme:
        failures.append("README is missing sandbox/VM profile contract command")
    if "sandbox-vm-profile-contract-check:" not in makefile:
        failures.append("Make target is missing: sandbox-vm-profile-contract-check")
    if "sandbox-vm-profile-contract-check" not in release_check_body:
        failures.append("sandbox/VM profile contract check is missing from release-check")
    if "sandbox-vm-profile-contract.md" not in enterprise:
        failures.append("enterprise runway is missing sandbox/VM profile contract pointer")
...
def render_report(report: dict[str, Any]) -> str:
    lines = [
        "Ithildin sandbox/VM profile contract check",
