"""Validate the project.config.summary implementation-planning packet."""
...
from scripts import project_config_summary_proposal_check, tool_surface_invariant_gate

ROOT = Path(__file__).resolve().parents[1]
PLAN_DOC = ROOT / "docs/codex/capability-implementation-plans/project-config-summary.md"
REQUIRED_PHRASES = [
...
FORBIDDEN_PHRASES = [
    ...
    "config parser execution is used",
...
def build_report(repo_root: Path) -> dict[str, Any]:
    ...
    if not plan_path.exists():
        return _report(["project.config.summary implementation plan is missing"], {})
    ...
    proposal = project_config_summary_proposal_check.build_report(repo_root)
...
def render_report(report: dict[str, Any]) -> str:
    lines = [
        "Ithildin project.config.summary implementation-plan check",
