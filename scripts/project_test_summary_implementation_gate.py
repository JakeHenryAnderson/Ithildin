"""Validate the project.test.summary implementation decision boundary."""
...
IMPLEMENTATION_DOC = ROOT / "docs/codex/v3-project-test-summary-implementation.md"
...
def build_report(repo_root: Path) -> dict[str, Any]:
    failures: list[str] = []
    plan_gate = project_test_summary_implementation_plan_check.build_report(repo_root)
    ...
    if not doc_path.exists():
        failures.append("project.test.summary implementation decision doc is missing")
    else:
    ...
    if not manifest_path.exists():
        failures.append("project.test.summary manifest is missing")
...
def render_report(report: dict[str, Any]) -> str:
    lines = [
        "Ithildin project.test.summary implementation gate",
