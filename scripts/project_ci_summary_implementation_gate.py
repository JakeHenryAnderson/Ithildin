"""Validate the project.ci.summary implementation decision boundary."""
...
IMPLEMENTATION_DOC = ROOT / "docs/codex/v3-project-ci-summary-implementation.md"
...
FORBIDDEN_DOC_PHRASES = [
    ...
    "workflow names are returned",
    "file contents are returned",
    "raw recursive listing is returned",
    "raw paths are returned",
    "command values are returned",
    "environment values are returned",
]
...
def build_report(repo_root: Path) -> dict[str, Any]:
    failures: list[str] = []
    plan_gate = project_ci_summary_implementation_plan_check.build_report(repo_root)
    ...
    if not doc_path.exists():
        failures.append("project.ci.summary implementation decision doc is missing")
    else:
    ...
    manifest_exists = repo_root.joinpath("tool-manifests/project-ci-summary.yaml").exists()
...
def render_report(report: dict[str, Any]) -> str:
    lines = [
        "Ithildin project.ci.summary implementation gate",
