"""Validate the project.manifest.summary implementation-planning packet."""
...
from scripts import project_manifest_summary_proposal_check, v09_design_only_gate

ROOT = Path(__file__).resolve().parents[1]
PLAN_DOC = ROOT / "docs/codex/capability-implementation-plans/project-manifest-summary.md"
REQUIRED_PHRASES = [
...
def build_report(repo_root: Path) -> dict[str, Any]:
    ...
    if not plan_path.exists():
        return _report(["project.manifest.summary implementation plan is missing"], {})
    ...
    proposal = project_manifest_summary_proposal_check.build_report(repo_root)
...
def render_report(report: dict[str, Any]) -> str:
    lines = [
        "Ithildin project.manifest.summary implementation-plan check",
