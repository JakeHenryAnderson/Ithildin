...
def build_check_report(repo_root: Path) -> dict[str, Any]:
    ...
    _require_phrases(
        ...
        [
            ...
            "send package/source artifacts reused",
            "response waiting room valid",
            "artifact_commits_match_current: `true`",
            "artifact_payloads_clean: `true`",
            "records_external_review: `false`",
            "normalizes_responses: `false`",
...
def _render_markdown(payload: dict[str, Any]) -> str:
    ...
    lines.extend(
        [
            ...
            "## Final freshness check",
            "",
            "Before sending from an already refreshed candidate, run "
            "`make enterprise-send-quick-check` and confirm it reports:",
            "",
            "- lightweight preflight valid",
            "- send package/source artifacts reused",
            "- upload staging valid",
            "- response waiting room valid",
            "",
            "If any artifact looks stale or you need full handoff regeneration, run "
            "`make enterprise-review-send-refresh`, then `make enterprise-review-send-preflight` "
            "and confirm it reports:",
            "",
            "- valid: `true`",
            "- current_dirty: `false`",
            "- artifact_commits_match_current: `true`",
