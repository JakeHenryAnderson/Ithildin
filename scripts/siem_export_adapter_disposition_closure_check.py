"""Validate the fail-closed SIEM export adapter disposition closure gate."""
...
DOC_REL = "docs/codex/siem-export-adapter-disposition-closure-gate.md"
DOC_NAME = "siem-export-adapter-disposition-closure-gate.md"
NORMALIZED_RESPONSE_REL = "var/review-runs/siem-export-adapter/normalized-response.json"
EXPECTED_AREA = "siem-export-adapter"
EXPECTED_NAMESPACE = "EXT-SIEM-ADAPTER-###"
...
FORBIDDEN_PHRASES = [
    ...
    "custody-grade audit is approved",
    "external notarization is approved",
    "immutable storage is approved",
...
def build_report(repo_root: Path) -> dict[str, Any]:
    ...
    intake = _read(repo_root / "docs/codex/siem-export-adapter-external-response-intake.md")
    disposition_packet = _read(repo_root / "docs/codex/siem-export-adapter-disposition-packet.md")
    architecture = _read(repo_root / "docs/codex/siem-export-adapter-architecture.md")
    release_check_body = makefile.partition("release-check:")[2].partition("\n\n")[0]

    if not doc_path.exists():
        failures.append("SIEM export adapter disposition closure gate doc is missing")
        text = ""
    else:
    ...
    for linked_text, source_name in [
        ...
        (disposition_packet, "SIEM export adapter disposition packet"),
        (architecture, "SIEM export adapter architecture"),
    ]:
        if DOC_NAME not in linked_text and DOC_REL not in linked_text:
            failures.append(f"{source_name} is missing {DOC_NAME}")
    if DOC_REL not in review_docs.REVIEW_DOCS:
        failures.append("review_docs.REVIEW_DOCS is missing SIEM export adapter closure gate doc")
    if "siem-export-adapter-disposition-closure-check:" not in makefile:
        failures.append("Make target is missing: siem-export-adapter-disposition-closure-check")
    release_check_additive = "release-check: siem-export-adapter-disposition-closure-check"
    if (
        "siem-export-adapter-disposition-closure-check" not in release_check_body
        and release_check_additive not in makefile
    ):
        failures.append("siem-export-adapter-disposition-closure-check missing from release-check")
    if "siem-export-adapter-disposition-closure-check" not in release_guardrails:
        failures.append("release guardrails do not require SIEM export adapter closure-check")
...
def _validate_normalized_response(path: Path) -> dict[str, Any]:
    if not path.exists():
        return {
            ...
            "reason": "normalized response is absent; ERG-008 remains planning-only",
        }
    failures: list[str] = []
    try:
    ...
    if payload.get("area") != EXPECTED_AREA:
        failures.append("normalized response area is not siem-export-adapter")
    ...
    for finding in findings:
        ...
        if not finding_id.startswith("EXT-SIEM-ADAPTER-"):
...
def render_report(report: dict[str, Any]) -> str:
    lines = [
        "Ithildin SIEM export adapter disposition closure check",
