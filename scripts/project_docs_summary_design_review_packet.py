"""Build the project.docs.summary design-review packet."""
...
DEFAULT_OUTPUT_DIR = Path("var/review-packets/v3/project-docs-summary-design-review")
...
DOCS = {
    ...
    "03_PROJECT_DOCS_SUMMARY_SELECTION.md": Path("docs/codex/v3-project-docs-summary-selection.md"),
...
class ProjectDocsSummaryDesignReviewPacketError(RuntimeError):
...
def main() -> int:
    ...
    except ProjectDocsSummaryDesignReviewPacketError as exc:
        print(f"project.docs.summary design-review packet failed: {exc}", file=sys.stderr)
        return 1

    print(f"Built project.docs.summary design-review packet at {output_dir}")
    return 0
...
def build_packet(*, repo_root: Path, output_dir: Path, allow_dirty: bool = False) -> Path:
    ...
    if dirty and not allow_dirty:
        raise ProjectDocsSummaryDesignReviewPacketError(
    ...
    if failures:
        raise ProjectDocsSummaryDesignReviewPacketError("; ".join(failures))
    ...
    _write_json(
        output_dir / "project-docs-summary-design-review-artifact-hashes.json",
        _hashes(output_dir),
    )
    return output_dir
...
def _index(context: dict[str, Any]) -> str:
    return f"""# project.docs.summary Design Review Packet
...
- Capability: `project.docs.summary`.
...
1. `00_PROJECT_DOCS_SUMMARY_DESIGN_REVIEW_INDEX.md`
2. `01_PROJECT_DOCS_SUMMARY_DESIGN_REVIEW_PROMPT.md`
...
behavior, runtime behavior, documentation build execution, documentation file names,
...
def _prompt(context: dict[str, Any]) -> str:
    return f"""# project.docs.summary Design Review Prompt
...
- whether count-only documentation metadata and allowlisted labels are useful enough without
  documentation file names, raw paths, file contents, documentation headings, scripts, coverage,
...
def _gate_evidence(context: dict[str, Any]) -> str:
    return f"""# project.docs.summary Gate And Risk Evidence
...
def _intake() -> str:
    return """# project.docs.summary Intake And Next Steps

If design review records findings, intake them with finding IDs `EXT-DESIGN-PDS-###` before any
...
def _git(repo_root: Path, args: list[str]) -> str:
    ...
    if process.returncode != 0:
        raise ProjectDocsSummaryDesignReviewPacketError(process.stderr.strip())
