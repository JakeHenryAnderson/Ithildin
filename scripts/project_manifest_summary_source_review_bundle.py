"""Build a focused project.manifest.summary source-review bundle."""
...
DEFAULT_OUTPUT_DIR = Path("var/review-packets/v0.9/project-manifest-summary-source-review")
...
SOURCE_FILES = [
    ...
    Path("tool-manifests/project-manifest-summary.yaml"),
...
CONTRACT_DOCS = [
    Path("docs/codex/v3-project-manifest-summary-implementation.md"),
    Path("docs/codex/capability-implementation-plans/project-manifest-summary.md"),
    Path("docs/codex/capability-proposals/project-manifest-summary.md"),
...
IMPLEMENTATION_GATE_COMMAND = ["make", "project-manifest-summary-implementation-gate"]
POLICY_PARITY_COMMAND = ["make", "policy-parity"]


class ProjectManifestSummarySourceReviewBundleError(RuntimeError):
...
def main() -> int:
    ...
    except ProjectManifestSummarySourceReviewBundleError as exc:
        print(f"project.manifest.summary source-review bundle failed: {exc}", file=sys.stderr)
        return 1

    print(f"Built project.manifest.summary source-review bundle at {output_dir}")
    return 0
...
def build_bundle(
    *,
    ...
    if dirty and not allow_dirty:
        raise ProjectManifestSummarySourceReviewBundleError(
    ...
    files = {
        "00_PROJECT_MANIFEST_SUMMARY_SOURCE_REVIEW_INDEX.md": _index(context),
        "01_PROJECT_MANIFEST_SUMMARY_SOURCE_REVIEW_PROMPT.md": _prompt(context),
        "02_PROJECT_MANIFEST_SUMMARY_IMPLEMENTATION_PACKET.md": implementation_packet,
        "03_PROJECT_MANIFEST_SUMMARY_SOURCE_BUNDLE.md": _bundle_sources(repo_root, SOURCE_FILES),
        "04_PROJECT_MANIFEST_SUMMARY_TESTS_BUNDLE.md": _bundle_sources(repo_root, TEST_FILES),
        "05_PROJECT_MANIFEST_SUMMARY_CONTRACTS_BUNDLE.md": _bundle_sources(
            repo_root, CONTRACT_DOCS
        ),
        "08_PROJECT_MANIFEST_SUMMARY_INTAKE_COMMANDS.md": _intake_commands(context),
    }
    ...
    (output_dir / "06_PROJECT_MANIFEST_SUMMARY_EVIDENCE.md").write_text(
        _packet_text(_evidence(gate_output, parity_output)),
        encoding="utf-8",
    )
    _write_command_output(
        output_dir / "07_PROJECT_MANIFEST_SUMMARY_FOCUSED_TESTS.txt",
        _command_output(FOCUSED_TEST_COMMAND, run_commands=run_commands),
    )
    _write_json(
        output_dir / "project-manifest-summary-source-review-artifact-hashes.json",
        _hashes(output_dir),
    )
    return output_dir
...
def _index(context: dict[str, Any]) -> str:
    return f"""# project.manifest.summary Source Review Handoff

This packet prepares the `project.manifest.summary` lane for source-level review. It attaches the
...
1. `00_PROJECT_MANIFEST_SUMMARY_SOURCE_REVIEW_INDEX.md`
2. `01_PROJECT_MANIFEST_SUMMARY_SOURCE_REVIEW_PROMPT.md`
3. `02_PROJECT_MANIFEST_SUMMARY_IMPLEMENTATION_PACKET.md`
4. `03_PROJECT_MANIFEST_SUMMARY_SOURCE_BUNDLE.md`
5. `04_PROJECT_MANIFEST_SUMMARY_TESTS_BUNDLE.md`
6. `05_PROJECT_MANIFEST_SUMMARY_CONTRACTS_BUNDLE.md`
7. `06_PROJECT_MANIFEST_SUMMARY_EVIDENCE.md`
8. `07_PROJECT_MANIFEST_SUMMARY_FOCUSED_TESTS.txt`
9. `08_PROJECT_MANIFEST_SUMMARY_INTAKE_COMMANDS.md`
10. `project-manifest-summary-source-review-artifact-hashes.json`
...
def _prompt(context: dict[str, Any]) -> str:
    return f"""# project.manifest.summary Source Review Prompt

You are reviewing Ithildin as a source reviewer for the `project.manifest.summary` lane only. Treat
...
- audit evidence containing only safe manifest count, truncation, parser status, manifest kind, and
  output-policy booleans;
- no-new-powers and tool-surface gates staying limited to this bounded read-only addition.

## Required Disposition

Please answer whether the `project.manifest.summary` lane can close for the v0.1 local-preview
...
| --- | --- | --- | --- |
| EXT-PMS-### | critical/high/medium/low/informational | project-manifest-summary | path/function |
...
def _implementation_packet(commit: str, dirty: bool, run_commands: bool) -> str:
    report = _command_output(
        [
            ...
            "run",
            "python",
            "scripts/project_manifest_summary_implementation_gate.py",
            "--json",
        ],
    ...
    return "\n".join(
        [
            "# project.manifest.summary Implementation Packet",
            "",
            f"- Reviewed commit: `{commit}`",
            f"- Dirty at generation: `{str(dirty).lower()}`",
            "- Tool: `project.manifest.summary`",
            "- Status: approved v3 implementation for one bounded read-only project manifest",
            "  metadata tool.",
...
def _evidence(gate_output: str, parity_output: str) -> str:
    return "\n".join(
        [
            "# project.manifest.summary Evidence",
            "",
            "## Implementation Gate",
            "",
            "```text",
...
def _intake_commands(context: dict[str, Any]) -> str:
    return f"""# project.manifest.summary Intake Commands

If a reviewer returns findings, normalize them with:

```bash
uv run python scripts/external_response_normalize.py \\
  var/review-runs/v0.9/project-manifest-summary/raw-response.md \\
  ...
  --area "project-manifest-summary" \\
  --output var/review-runs/v0.9/project-manifest-summary/normalized-response.json
```
...
def _bundle_sources(repo_root: Path, paths: list[Path]) -> str:
    ...
    for path in paths:
        ...
        if not source_path.exists():
            raise ProjectManifestSummarySourceReviewBundleError(f"missing bundle input: {path}")
        sections.extend(
            [
                f"# {path.as_posix()}",
                "",
...
def _hashes(output_dir: Path) -> list[dict[str, Any]]:
    ...
    hash_manifest = "project-manifest-summary-source-review-artifact-hashes.json"
