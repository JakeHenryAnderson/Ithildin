"""Validate the ERG-004 runtime descriptor-only implementation-planning packet."""
...
DOC_REL = "docs/codex/sandbox-vm-live-poc-runtime-descriptor-only-plan.md"
DOC_TITLE = "Sandbox/VM Live POC Runtime Descriptor-Only Plan"
TARGET = "sandbox-vm-live-poc-runtime-descriptor-only-plan-check"

REQUIRED_PHRASES = [
    (
        "Status: descriptor-only implementation-planning packet for the future "
        "`ERG-004` runtime descriptor"
    ),
...
def build_report(repo_root: Path) -> dict[str, Any]:
    ...
    if not doc_path.exists():
        failures.append("runtime descriptor-only plan doc is missing")
    for phrase in REQUIRED_PHRASES:
        if phrase not in text:
            failures.append(f"runtime descriptor-only plan is missing phrase: {phrase}")
    for phrase in REQUIRED_NEGATIVES:
        if phrase not in text:
            failures.append(f"runtime descriptor-only plan is missing negative case: {phrase}")
    for phrase in REQUIRED_NON_APPROVALS:
        if phrase not in text:
            failures.append(f"runtime descriptor-only plan is missing non-approval: {phrase}")
    for phrase in FORBIDDEN_PHRASES:
        if phrase.lower() in lowered:
            failures.append(f"runtime descriptor-only plan contains forbidden phrase: {phrase}")
    ...
    if DOC_REL not in review_docs.REVIEW_DOCS:
        failures.append("runtime descriptor-only plan is missing from review docs")
    if DOC_REL not in docs_site:
        failures.append("runtime descriptor-only plan is missing from docs-site inputs")
    if DOC_TITLE not in review_index:
        failures.append("review-docs index is missing runtime descriptor-only plan")
    if f"{TARGET}:" not in makefile:
        failures.append(f"Make target is missing: {TARGET}")
    if TARGET not in release_check_body:
        failures.append("runtime descriptor-only plan check missing from release-check")
    if TARGET not in release_guardrails:
        failures.append("release guardrails do not require runtime descriptor-only plan check")
    if f"make {TARGET}" not in readme:
        failures.append("README is missing runtime descriptor-only plan command")
    if DOC_REL not in readme:
        failures.append("README is missing runtime descriptor-only plan doc")

    return {
        "schema_version": "1",
        "valid": not failures,
        "failures": failures,
        "runtime_descriptor_only_plan_doc": DOC_REL,
        "tool_count": 24,
...
def render_report(report: dict[str, Any]) -> str:
    lines = [
        "Ithildin sandbox/VM live POC runtime descriptor-only plan check",
        f"valid: {str(report['valid']).lower()}",
        f"runtime_descriptor_only_plan_doc: {report['runtime_descriptor_only_plan_doc']}",
