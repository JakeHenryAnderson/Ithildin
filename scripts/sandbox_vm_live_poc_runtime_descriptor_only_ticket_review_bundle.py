"""Build the ERG-004 descriptor-only implementation-ticket review bundle."""
...
HASH_MANIFEST = "sandbox-vm-live-poc-runtime-descriptor-only-ticket-artifact-hashes.json"
DOC_REL = "docs/codex/sandbox-vm-live-poc-runtime-descriptor-only-ticket-review-bundle.md"
DOC_TITLE = "Sandbox/VM Live POC Runtime Descriptor-Only Ticket Review Bundle"
TARGET = "sandbox-vm-live-poc-runtime-descriptor-only-ticket-review-bundle"
CHECK_TARGET = "sandbox-vm-live-poc-runtime-descriptor-only-ticket-review-bundle-check"
PROJECT_MARKERS = [
    Path("pyproject.toml"),
    Path("Makefile"),
    Path("apps/api"),
...
ARTIFACTS = [
    "00_SANDBOX_VM_LIVE_POC_DESCRIPTOR_ONLY_TICKET_REVIEW_INDEX.md",
    "01_SANDBOX_VM_LIVE_POC_DESCRIPTOR_ONLY_TICKET_REVIEW_PROMPT.md",
    "02_ERG004_DESCRIPTOR_ONLY_PLAN_AND_TICKET.md",
    "03_ERG004_DESCRIPTOR_CONTRACT_AND_GATE_CONTEXT.md",
    "04_ERG004_DESCRIPTOR_ONLY_NEGATIVE_AND_ACCEPTANCE_PLAN.md",
...
class DescriptorOnlyTicketReviewBundleError(RuntimeError):
    """Raised when the descriptor-only ticket review bundle cannot be built."""
...
def build_bundle(
    *,
    ...
    if dirty and not allow_dirty:
        raise DescriptorOnlyTicketReviewBundleError(
            "working tree is dirty; commit before ERG-004 descriptor-only ticket handoff"
        )
    ...
    files = {
        "00_SANDBOX_VM_LIVE_POC_DESCRIPTOR_ONLY_TICKET_REVIEW_INDEX.md": _index(
            commit, dirty
        ),
        "01_SANDBOX_VM_LIVE_POC_DESCRIPTOR_ONLY_TICKET_REVIEW_PROMPT.md": _prompt(
            commit
        ),
        "02_ERG004_DESCRIPTOR_ONLY_PLAN_AND_TICKET.md": _docs_bundle(
            "ERG-004 Descriptor-Only Plan And Implementation Ticket",
            repo_root,
            [
        ...
        "03_ERG004_DESCRIPTOR_CONTRACT_AND_GATE_CONTEXT.md": _docs_bundle(
            "ERG-004 Descriptor Contract And Gate Context",
            repo_root,
            [
        ...
        "04_ERG004_DESCRIPTOR_ONLY_NEGATIVE_AND_ACCEPTANCE_PLAN.md": _docs_bundle(
            "ERG-004 Descriptor-Only Negative And Acceptance Plan",
            repo_root,
            [
...
def build_check_report(repo_root: Path) -> dict[str, Any]:
    ...
    with tempfile.TemporaryDirectory() as tmp:
        output_dir = Path(tmp) / "descriptor-only-ticket-review"
        try:
            build_bundle(
                repo_root=repo_root,
        ...
        except DescriptorOnlyTicketReviewBundleError as exc:
    ...
    if missing:
        failures.append(
            "descriptor-only ticket review bundle missing artifacts: "
            + ", ".join(sorted(missing))
        )
    ...
    plan_ticket = contents.get("02_ERG004_DESCRIPTOR_ONLY_PLAN_AND_TICKET.md", "")
    context = contents.get("03_ERG004_DESCRIPTOR_CONTRACT_AND_GATE_CONTEXT.md", "")
    negatives = contents.get("04_ERG004_DESCRIPTOR_ONLY_NEGATIVE_AND_ACCEPTANCE_PLAN.md", "")
    ...
    for name, content in [
        ...
        if current_commit not in content:
            failures.append(f"descriptor-only ticket {name} does not reference current commit")
    ...
    for phrase in [
        ...
        if phrase not in index:
            failures.append(f"descriptor-only ticket index is missing phrase: {phrase}")
    for phrase in [
        ...
        if phrase not in prompt:
            failures.append(f"descriptor-only ticket prompt is missing phrase: {phrase}")
    for phrase in [
        "sandbox-vm-live-poc-runtime-descriptor-only-plan.md",
        ...
        if phrase not in plan_ticket:
            failures.append(f"descriptor-only plan/ticket bundle is missing phrase: {phrase}")
    for phrase in [
        ...
        if phrase not in context:
            failures.append(f"descriptor-only context bundle is missing phrase: {phrase}")
    for phrase in [
        "sandbox-vm-live-poc-runtime-negative-fixtures.md",
        ...
        if phrase not in negatives:
            failures.append(f"descriptor-only negative bundle is missing phrase: {phrase}")
    for phrase in [
        '"sandbox-vm-live-poc-runtime-descriptor-only-plan-check"',
        '"sandbox-vm-live-poc-runtime-descriptor-only-implementation-ticket-check"',
        ...
        if phrase not in evidence:
            failures.append(f"descriptor-only command evidence is missing phrase: {phrase}")

    combined = "\n".join(contents.values())
    for phrase in FORBIDDEN_APPROVAL_PHRASES:
        if phrase in combined:
            failures.append(f"descriptor-only ticket bundle contains forbidden phrase: {phrase}")

    if DOC_REL not in docs_site:
        failures.append("descriptor-only ticket review doc is missing from docs site")
    if DOC_REL not in review_docs:
        failures.append("descriptor-only ticket review doc is missing from review docs")
    if DOC_TITLE not in review_index:
        failures.append("review-docs index is missing descriptor-only ticket review bundle")
    ...
    if CHECK_TARGET not in release_check_body and f"release-check: {CHECK_TARGET}" not in makefile:
        failures.append("descriptor-only ticket review check missing from release-check")
    if f"$(MAKE) {TARGET}" not in review_candidate_body:
        failures.append("review-candidate does not generate descriptor-only ticket bundle")
    if CHECK_TARGET not in release_guardrails:
        failures.append("release guardrails do not require descriptor-only ticket check")
    if f"$(MAKE) {TARGET}" not in release_guardrails:
        failures.append("release guardrails do not require descriptor-only ticket generation")
    if f"make {TARGET}" not in readme:
        failures.append("README is missing descriptor-only ticket bundle command")
    ...
    return {
        ...
        "descriptor_only_ticket_review_allowed": True,
...
def render_check_report(report: dict[str, Any]) -> str:
    lines = [
        "Ithildin sandbox/VM live POC descriptor-only ticket review bundle check",
        ...
        "descriptor_only_ticket_review_allowed: "
        f"{str(report['descriptor_only_ticket_review_allowed']).lower()}",
...
def _index(commit: str, dirty: bool) -> str:
    return f"""# Sandbox/VM Live POC Descriptor-Only Ticket Review Index
...
This packet packages the descriptor-only implementation plan and ticket, descriptor contract, gate
context, negative fixture expectations, and command evidence. It asks whether a later
descriptor-only runtime implementation sprint may be planned from this ticket.
...
This bundle does not approve runtime implementation, does not inspect or control a VM/container,
does not invoke a local model, does not grant Mission Control runtime authority, does not write or
promote host artifacts, does not load profiles through API/MCP, and does not close `ERG-004`.
"""


def _prompt(commit: str) -> str:
    return f"""# ERG-004 Descriptor-Only Ticket Review Prompt

You are reviewing Ithildin's `ERG-004` live sandbox/VM POC lane at the descriptor-only
implementation-ticket stage. Treat this as a packet review only unless you inspect the attached
docs/evidence.

Reviewed commit: `{commit}`

Area: `sandbox-vm-live-poc-runtime-descriptor-only-ticket`

Finding namespace: `EXT-LIVE-DESC-###`

## Question

Can a descriptor-only runtime implementation sprint may be planned later from this ticket, limited
to operator-supplied descriptor validation and correlation evidence?

## Review Focus

- Does the ticket preserve operator-managed VM lifecycle and OS isolation?
- Are descriptor facts treated as operator-supplied attestations rather than Ithildin-verified VM
  state?
- Are future surfaces explicitly undecided until a later implementation sprint?
- Do negative cases block lifecycle control, live inspection, local model invocation, Mission
  Control authority, trusted-host promotion, host writes, network expansion, and API/MCP profile
  loading?
- Does the packet require source-review handoff after any descriptor-only implementation exists?

## Required Disposition

Answer whether a later descriptor-only runtime implementation sprint may be planned from this
ticket. If not, list the exact blocker or missing evidence.
...
def _negative_expectations() -> str:
    return """## Descriptor-Only Negative Review Expectations
...
- raw secret, prompt, model response, file content, diff, transcript, dependency name, package
  script value, raw path, command line, shell output, or directory listing leakage.
"""
...
def _command_evidence(
    ...
    return "# ERG-004 Descriptor-Only Ticket Command Evidence\n\n```json\n" + json.dumps(
...
def _build_command_reports(repo_root: Path, *, run_commands: bool) -> list[dict[str, Any]]:
    ...
    for name, cmd in COMMANDS:
        ...
        if result.returncode != 0:
            raise DescriptorOnlyTicketReviewBundleError(
                f"command failed for descriptor-only ticket review bundle: {name}"
            )
    return reports
...
def main() -> int:
    ...
    try:
        ...
        return 1
    print(f"Built ERG-004 descriptor-only ticket review bundle at {output_dir}")
    return 0
