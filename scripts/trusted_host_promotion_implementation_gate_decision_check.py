"""Validate the committed ERG-005 trusted-host Goal C implementation-gate decision."""
...
DOC_REL = "docs/codex/trusted-host-promotion-implementation-gate-decision.md"
DOC_NAME = "trusted-host-promotion-implementation-gate-decision.md"
TARGET = "trusted-host-promotion-implementation-gate-decision-check"
...
REQUIRED_PHRASES = [
    ...
    "approval consumption and one-time scope",
    "diagnostics must distinguish no-write, post-placement ambiguous, replay",
...
def build_report(repo_root: Path) -> dict[str, Any]:
    ...
    readiness = _read(
        repo_root / "docs/codex/enterprise-sandbox-control-plane-readiness.md"
    )
    operator_next = _read(repo_root / "docs/codex/enterprise-operator-next-action.md")
    release_check_body = makefile.partition("release-check:")[2].partition("\n\n")[0]

    if not doc_path.exists():
        failures.append("trusted-host Goal C implementation-gate decision doc is missing")
    for phrase in REQUIRED_PHRASES:
        if phrase not in normalized_text and phrase not in text:
            failures.append(f"Goal C decision doc is missing phrase: {phrase}")
    for phrase in REQUIRED_BLOCKED_BOUNDARIES:
        if phrase not in text:
            failures.append(f"Goal C decision doc is missing blocked boundary: {phrase}")
    for phrase in FORBIDDEN_PHRASES:
        if phrase.lower() in lowered:
            failures.append(f"Goal C decision doc contains forbidden phrase: {phrase}")

    upstream_reports = [
        ("decision-record", trusted_host_promotion_decision_record_check.build_report(repo_root)),
        (
            "implementation-plan",
        ...
        ("internal-review", trusted_host_promotion_internal_review_check.build_report(repo_root)),
    ...
    closure_gate = dict(upstream_reports[7][1])
    internal_review = dict(upstream_reports[3][1])
    state_machine = dict(upstream_reports[4][1])
    negative_fixtures = dict(upstream_reports[5][1])
    tool_surface = dict(upstream_reports[8][1])

    if closure_gate.get("normalized_response_present") is not False:
        failures.append("Goal C expects no live normalized trusted-host response")
    ...
    for linked_text, source_name in [
        ...
        (operator_next, "enterprise operator next action"),
    ]:
        if DOC_NAME not in linked_text and DOC_REL not in linked_text:
            failures.append(f"{source_name} is missing {DOC_NAME}")
    if DOC_REL not in review_docs.REVIEW_DOCS:
        failures.append("Goal C decision doc is missing from review docs")
    if "Trusted-Host Promotion Implementation-Gate Decision" not in review_index:
        failures.append("review-docs index is missing trusted-host Goal C decision")
    if f"{TARGET}:" not in makefile:
        failures.append(f"Make target is missing: {TARGET}")
    if TARGET not in release_check_body:
        failures.append("Goal C decision check missing from release-check")
    if TARGET not in release_guardrails:
        failures.append("release guardrails do not require Goal C decision check")
    if f"make {TARGET}" not in readme:
        failures.append("README is missing Goal C decision command")
    ...
    return {
        ...
        "decision_id": "PRD-TRUSTED-HOST-GATE-001",
        "tool_count": 24,
        "previous_erg_005_status": "ready_for_implementation_planning_only",
        "goal_c_outcome": "ready_for_limited_runtime_implementation_plan",
        "next_allowed_step": "limited_runtime_implementation_plan",
...
def render_report(report: dict[str, Any]) -> str:
    lines = [
        "Ithildin trusted-host promotion implementation-gate decision check",
        f"valid: {str(report['valid']).lower()}",
        f"decision_doc: {report['decision_doc']}",
        f"decision_id: {report['decision_id']}",
        f"tool_count: {report['tool_count']}",
        f"previous_erg_005_status: {report['previous_erg_005_status']}",
