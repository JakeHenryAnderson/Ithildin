"""Validate the project.config.summary implementation decision boundary."""
...
IMPLEMENTATION_DOC = ROOT / "docs/codex/v3-project-config-summary-implementation.md"
...
def build_report(repo_root: Path) -> dict[str, Any]:
    failures: list[str] = []
    plan_gate = project_config_summary_implementation_plan_check.build_report(repo_root)
    ...
    if not doc_path.exists():
        failures.append("project.config.summary implementation decision doc is missing")
    else:
    ...
    manifest_exists = repo_root.joinpath("tool-manifests/project-config-summary.yaml").exists()
...
def render_report(report: dict[str, Any]) -> str:
    lines = [
        "Ithildin project.config.summary implementation gate",
