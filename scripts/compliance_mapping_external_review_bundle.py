"""Build a reviewer-friendly ERG-009 compliance mapping bundle."""
...
DEFAULT_OUTPUT_DIR = Path("var/review-packets/v3/compliance-mapping-external-review")
HASH_MANIFEST = "compliance-mapping-external-review-artifact-hashes.json"
PROJECT_MARKERS = [
    Path("pyproject.toml"),
    Path("Makefile"),
    Path("apps/api"),
...
class ComplianceMappingExternalReviewBundleError(RuntimeError):
...
def main() -> int:
    ...
    except ComplianceMappingExternalReviewBundleError as exc:
        print(f"compliance mapping external-review bundle failed: {exc}", file=sys.stderr)
        return 1

    print(f"Built compliance mapping external-review bundle at {output_dir}")
    return 0
...
def build_check_report(repo_root: Path) -> dict[str, Any]:
    ...
    with tempfile.TemporaryDirectory() as tmp:
        output_dir = Path(tmp) / "compliance-mapping-external-review"
        try:
            build_bundle(
                repo_root=repo_root,
    ...
    index = contents.get("00_COMPLIANCE_MAPPING_EXTERNAL_REVIEW_INDEX.md", "")
    prompt = contents.get("01_COMPLIANCE_MAPPING_EXTERNAL_REVIEW_PROMPT.md", "")
    response = contents.get("04_COMPLIANCE_MAPPING_RESPONSE_CLOSURE_DRY_RUN.md", "")
    reproduction = contents.get("05_COMPLIANCE_MAPPING_REPRODUCTION_QUEUE_STATUS.md", "")
    evidence = contents.get("07_COMPLIANCE_MAPPING_COMMAND_EVIDENCE.md", "")

    for phrase in [
        "Finding namespace: `EXT-COMPLIANCE-MAPPING-###`",
        "Can `ERG-009` continue architecture planning",
        "Do not approve compliance mapping runtime behavior",
        "Do not approve legal advice",
    ]:
    ...
    target = "compliance-mapping-external-review-bundle"
    ...
    doc_rel = "docs/codex/compliance-mapping-external-review-bundle.md"
    ...
    if "Compliance Mapping External Review Bundle" not in review_index:
    ...
    for text, source in [
        ...
        if "compliance-mapping-external-review-bundle" not in text:
...
def build_bundle(
    *,
    ...
    if dirty and not allow_dirty:
        raise ComplianceMappingExternalReviewBundleError(
    ...
    with tempfile.TemporaryDirectory() as tmp:
        ...
        files = {
            "00_COMPLIANCE_MAPPING_EXTERNAL_REVIEW_INDEX.md": _index(commit, dirty),
            "01_COMPLIANCE_MAPPING_EXTERNAL_REVIEW_PROMPT.md": _prompt(commit),
            "02_COMPLIANCE_MAPPING_DISPOSITION_PACKET.md": _packet_bundle(
                "Disposition Packet Contents", disposition_dir
            ),
            "03_COMPLIANCE_MAPPING_ARCHITECTURE_CONTRACTS.md": _docs_bundle(
            ...
            "04_COMPLIANCE_MAPPING_RESPONSE_CLOSURE_DRY_RUN.md": _docs_bundle(
            ...
            "05_COMPLIANCE_MAPPING_REPRODUCTION_QUEUE_STATUS.md": _docs_bundle(
            ...
            "06_COMPLIANCE_MAPPING_BOUNDARY_EVIDENCE.md": _docs_bundle(
            ...
            "07_COMPLIANCE_MAPPING_COMMAND_EVIDENCE.md": _command_evidence(command_reports),
        }
...
def _index(commit: str, dirty: bool) -> str:
    return f"""# Compliance Mapping External Review Bundle

Status: reviewer launch bundle for `ERG-009`.
...
Recommended next review: `ERG-009` compliance mapping architecture disposition.

## Reading Order

1. `01_COMPLIANCE_MAPPING_EXTERNAL_REVIEW_PROMPT.md`
2. `02_COMPLIANCE_MAPPING_DISPOSITION_PACKET.md`
3. `03_COMPLIANCE_MAPPING_ARCHITECTURE_CONTRACTS.md`
4. `04_COMPLIANCE_MAPPING_RESPONSE_CLOSURE_DRY_RUN.md`
5. `05_COMPLIANCE_MAPPING_REPRODUCTION_QUEUE_STATUS.md`
6. `06_COMPLIANCE_MAPPING_BOUNDARY_EVIDENCE.md`
7. `07_COMPLIANCE_MAPPING_COMMAND_EVIDENCE.md`
8. `compliance-mapping-external-review-artifact-hashes.json`

## What This Bundle Does Not Prove

This bundle does not prove that external review has happened, does not close `ERG-009`, and does not
approve compliance mapping runtime behavior, compliance automation, legal advice, legal conclusions,
automated certification, regulated-industry compliance claims, custody-grade audit claims,
production identity, runtime Postgres, SIEM adapter runtime behavior, hosted control plane, remote
...
SDK behavior, public/security-product positioning, or any new governed tool power.

This bundle does not approve compliance mapping runtime behavior.
"""


def _prompt(commit: str) -> str:
    return f"""# Compliance Mapping External Review Prompt

You are reviewing Ithildin as an external/source reviewer for `ERG-009` only: the compliance
mapping support architecture lane.
...
Review the attached disposition packet, compliance mapping architecture contract, control mapping
design, data classification design, incident reconstruction guide, decision-register pointers,
external response intake, closure gate, dry-run evidence, queue status, and command evidence.

Please answer:

1. Did you inspect the compliance mapping disposition packet and architecture packet?
2. Are the mapping template, target framework/control-family scope, control objective taxonomy,
   confidence labels, false-assurance warnings, and legal-review boundary complete enough for
   continued architecture planning?
3. Are the evidence allowlist/denylist, data classification inputs, accepted-risk references,
   verification pointers, and operator responsibility model complete enough for continued
   architecture planning?
4. Are the secret-free evidence fields safe and useful for future control-mapping support and
...
7. Can `ERG-009` continue architecture planning while runtime implementation remains blocked?

Do not approve compliance mapping runtime behavior. Do not approve compliance automation.
Do not approve legal advice. Do not approve legal conclusions. Do not approve automated
certification. Do not approve regulated-industry compliance claims. Do not approve custody-grade
audit claims. Do not approve production identity. Do not approve runtime Postgres. Do not approve
SIEM adapter runtime behavior. Do not approve sandbox orchestration. Do not approve trusted-host
promotion. Do not approve public/security-product positioning.

Use this finding namespace for actionable findings: `EXT-COMPLIANCE-MAPPING-###`.
...
def _build_command_reports(repo_root: Path, *, run_commands: bool) -> dict[str, Any]:
    reports: dict[str, Any] = {
        ...
        "disposition_packet_check": compliance_mapping_disposition_packet.build_check_report(
            repo_root
        ),
        "external_response_intake_check": (
            compliance_mapping_external_response_intake_check.build_report(repo_root)
        ),
        "closure_gate_check": compliance_mapping_disposition_closure_check.build_report(
            repo_root
        ),
...
def render_check_report(report: dict[str, Any]) -> str:
    lines = [
        "Ithildin compliance mapping external-review bundle check",
        ...
        f"legal_conclusions_allowed: {str(report['legal_conclusions_allowed']).lower()}",
