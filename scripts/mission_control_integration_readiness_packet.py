"""Build a Mission Control integration readiness packet."""
...
DEFAULT_OUTPUT_DIR = Path("var/review-packets/v3/mission-control-integration-readiness")
HASH_MANIFEST = "mission-control-integration-readiness-artifact-hashes.json"
PROJECT_MARKERS = [
    Path("pyproject.toml"),
    Path("Makefile"),
    Path("apps/api"),
...
READINESS_DOCS = [
    Path("docs/codex/mission-control-integration-readiness-packet.md"),
...
FINDING_TABLE = "\n".join(
    [
        ...
        "| --- | --- | --- | --- | --- | --- | --- |",
        "| EXT-MC-INTEGRATION-### | critical/high/medium/low/informational | "
        "mission-control-integration | path/function | "
        "blocking/should-fix/later/advisory | open | fix summary |",
    ]
)


class MissionControlIntegrationReadinessPacketError(RuntimeError):
    """Raised when the Mission Control integration readiness packet cannot be built."""
...
def main() -> int:
    ...
    except MissionControlIntegrationReadinessPacketError as exc:
        print(f"Mission Control integration readiness packet failed: {exc}", file=sys.stderr)
        return 1

    print(f"Built Mission Control integration readiness packet at {output_dir}")
    return 0
...
def build_check_report(repo_root: Path) -> dict[str, Any]:
    ...
    doc_rel = "docs/codex/mission-control-integration-readiness-packet.md"
    doc_text = _read(repo_root / doc_rel)
    for phrase in [
        "Status: planning-only cross-repo readiness packet",
        ...
        if phrase not in doc_text:
            failures.append(f"readiness doc missing phrase: {phrase}")

    with tempfile.TemporaryDirectory() as tmp:
        output_dir = Path(tmp) / "mission-control-integration-readiness"
        try:
            build_packet(
                repo_root=repo_root,
            ...
            prompt = (
                output_dir / "01_MISSION_CONTROL_INTEGRATION_READINESS_PROMPT.md"
            ).read_text(encoding="utf-8")
            index = (
                output_dir / "00_MISSION_CONTROL_INTEGRATION_READINESS_INDEX.md"
            ).read_text(encoding="utf-8")
            evidence = (
                output_dir / "04_MISSION_CONTROL_INTEGRATION_COMMAND_EVIDENCE.md"
            ).read_text(encoding="utf-8")
    ...
    for phrase in [
        "Finding namespace: `EXT-MC-INTEGRATION-###`",
    ...
    if "mission-control-integration-readiness-packet-check" not in release_check_body:
        failures.append("Mission Control integration readiness check missing from release-check")
    if "$(MAKE) mission-control-integration-readiness-packet" not in review_candidate_body:
        failures.append("Mission Control integration readiness missing from review-candidate")
    if "mission-control-integration-readiness-packet-check" not in release_guardrails:
        failures.append("release guardrails do not require Mission Control integration check")
    if "$(MAKE) mission-control-integration-readiness-packet" not in release_guardrails:
        failures.append("release guardrails do not require Mission Control integration packet")
    if doc_rel not in docs_site:
        failures.append(
            "Mission Control integration readiness doc is missing from docs-site inputs"
        )
    if doc_rel not in review_docs:
        failures.append("Mission Control integration readiness doc is missing from review docs")
    if "Mission Control Integration Readiness Packet" not in review_index:
        failures.append("review-docs index is missing Mission Control integration readiness entry")
    if "make mission-control-integration-readiness-packet" not in readme:
        failures.append("README is missing Mission Control integration readiness command")
    if "mission-control-integration-readiness-packet.md" not in runway:
        failures.append(
            "enterprise runway is missing Mission Control integration readiness pointer"
        )
    if "mission-control-integration-readiness-packet.md" not in gap_matrix:
        failures.append(
            "enterprise gap matrix is missing Mission Control integration readiness pointer"
        )
    if "mission-control-integration-readiness-packet.md" not in decision_register:
        failures.append("post-RC decision register is missing Mission Control readiness pointer")
...
def build_packet(
    *,
    ...
    if dirty and not allow_dirty:
        raise MissionControlIntegrationReadinessPacketError(
            "working tree is dirty; commit before integration readiness handoff"
        )

    reports = _collect_reports(repo_root)
    failures = [
        f"{key}: {failure}"
    ...
    if failures:
        raise MissionControlIntegrationReadinessPacketError("; ".join(failures))
    ...
    files = {
        "00_MISSION_CONTROL_INTEGRATION_READINESS_INDEX.md": _index(context),
        "01_MISSION_CONTROL_INTEGRATION_READINESS_PROMPT.md": _prompt(context),
        "02_MISSION_CONTROL_INTEGRATION_DOCS.md": _bundle_docs(repo_root, READINESS_DOCS),
        "03_MISSION_CONTROL_CONTEXT_DOCS.md": _bundle_docs(repo_root, CONTEXT_DOCS),
        "04_MISSION_CONTROL_INTEGRATION_COMMAND_EVIDENCE.md": _command_evidence(
            repo_root=repo_root,
            run_commands=run_commands,
            reports=reports,
        ),
...
def _collect_reports(repo_root: Path) -> dict[str, dict[str, Any]]:
    return {
        "integration_proposal_check": (
            mission_control_display_integration_proposal_check.build_report(repo_root)
        ),
        "importer_plan_check": mission_control_display_importer_plan_check.build_report(repo_root),
        "decision_intake_check": mission_control_display_decision_intake_check.build_report(
            repo_root
        ),
        "review_packet_check": mission_control_display_review_packet.build_check_report(repo_root),
        "disposition_packet_check": mission_control_display_disposition_packet.build_check_report(
            repo_root
        ),
        "side_handoff_plan_check": mission_control_side_handoff_plan_check.build_report(repo_root),
        "implementation_ticket_check": (
            mission_control_integration_implementation_ticket_check.build_report(repo_root)
        ),
        "handoff_schema_contract_check": mission_control_handoff_schema_contract_check.build_report(
            repo_root
        ),
        "handoff_negative_fixtures_check": (
            mission_control_handoff_negative_fixtures_check.build_report(repo_root)
        ),
...
def _index(context: dict[str, Any]) -> str:
    return f"""# Mission Control Integration Readiness Packet

This packet consolidates the Ithildin-side evidence for the Mission Control display/importer lane.
It is a handoff packet for the future Mission Control repository task, not an Ithildin runtime
integration packet.
...
1. `00_MISSION_CONTROL_INTEGRATION_READINESS_INDEX.md`
2. `01_MISSION_CONTROL_INTEGRATION_READINESS_PROMPT.md`
3. `02_MISSION_CONTROL_INTEGRATION_DOCS.md`
4. `03_MISSION_CONTROL_CONTEXT_DOCS.md`
5. `04_MISSION_CONTROL_INTEGRATION_COMMAND_EVIDENCE.md`
6. `mission-control-integration-readiness-artifact-hashes.json`

## What This Packet Does Not Prove

This packet does not prove Mission Control source implementation correctness, does not prove a
runtime importer exists, does not prove bidirectional integration, does not close `ERG-002`, and
does not approve Mission Control runtime importer behavior. A later Mission Control-side source
review packet and post-RC decision record must disposition implementation readiness before Mission
Control runtime work proceeds.
"""


def _prompt(context: dict[str, Any]) -> str:
    return f"""# Mission Control Integration Readiness Review Prompt

You are reviewing Ithildin's Mission Control display/importer integration readiness packet.
...
1. Is the Mission Control-side implementation ticket complete enough to hand to the Mission Control
   repository as a display-only import/display task?
...
- `continue_display_importer_planning`: current evidence is coherent for a Mission Control-side
  display/import implementation plan and source-review packet.
- `revise_handoff_before_implementation`: gaps or ambiguous claims must be fixed before handing
  this to Mission Control implementation.
- `block_mission_control_runtime_work`: a blocking risk prevents Mission Control runtime importer
  work until a new decision record resolves it.
...
def _command_evidence(
    *,
    ...
    lines = [
        "# Mission Control Integration Readiness Command Evidence",
        "",
        "## Boundary Flags",
        "",
        "```json",
...
def _bundle_docs(repo_root: Path, docs: list[Path]) -> str:
    ...
    for doc in docs:
        path = repo_root / doc
        if not path.exists():
            raise MissionControlIntegrationReadinessPacketError(f"missing doc: {doc}")
...
def render_check_report(report: dict[str, Any]) -> str:
    lines = [
        "Ithildin Mission Control integration readiness packet check",
