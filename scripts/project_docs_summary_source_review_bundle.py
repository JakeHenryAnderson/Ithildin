"""Build a focused project.docs.summary source-review bundle."""
...
DEFAULT_OUTPUT_DIR = Path("var/review-packets/v3/project-docs-summary-source-review")
SOURCE_FILES = [
    ...
    Path("tool-manifests/project-docs-summary.yaml"),
...
CONTRACT_DOCS = [
    Path("docs/codex/v3-project-docs-summary-implementation.md"),
    Path("docs/codex/capability-implementation-plans/project-docs-summary.md"),
    Path("docs/codex/capability-proposals/project-docs-summary.md"),
...
IMPLEMENTATION_GATE_COMMAND = ["make", "project-docs-summary-implementation-gate"]
POLICY_PARITY_COMMAND = ["make", "policy-parity"]


class ProjectDocsSummarySourceReviewBundleError(RuntimeError):
...
def main() -> int:
    ...
    except ProjectDocsSummarySourceReviewBundleError as exc:
        print(f"project.docs.summary source-review bundle failed: {exc}", file=sys.stderr)
        return 1
    print(f"Built project.docs.summary source-review bundle at {output_dir}")
    return 0
...
def build_bundle(
    *,
    ...
    if dirty and not allow_dirty:
        raise ProjectDocsSummarySourceReviewBundleError(
    ...
    files = {
        "00_PROJECT_DOCS_SUMMARY_SOURCE_REVIEW_INDEX.md": _index(context),
        "01_PROJECT_DOCS_SUMMARY_SOURCE_REVIEW_PROMPT.md": _prompt(context),
        "02_PROJECT_DOCS_SUMMARY_IMPLEMENTATION_PACKET.md": implementation_packet,
        "03_PROJECT_DOCS_SUMMARY_SOURCE_BUNDLE.md": _bundle_sources(repo_root, SOURCE_FILES),
        "04_PROJECT_DOCS_SUMMARY_TESTS_BUNDLE.md": _bundle_sources(repo_root, TEST_FILES),
        "05_PROJECT_DOCS_SUMMARY_CONTRACTS_BUNDLE.md": _bundle_sources(
            repo_root, CONTRACT_DOCS
        ),
        "08_PROJECT_DOCS_SUMMARY_INTAKE_COMMANDS.md": _intake_commands(),
    }
    ...
    (output_dir / "06_PROJECT_DOCS_SUMMARY_EVIDENCE.md").write_text(
        _packet_text(_evidence(gate_output, parity_output)),
        encoding="utf-8",
    )
    _write_command_output(
        output_dir / "07_PROJECT_DOCS_SUMMARY_FOCUSED_TESTS.txt",
        _command_output(FOCUSED_TEST_COMMAND, run_commands=run_commands),
    )
    _write_json(
        output_dir / "project-docs-summary-source-review-artifact-hashes.json",
        _hashes(output_dir),
    )
    return output_dir
...
def _index(context: dict[str, Any]) -> str:
    return f"""# project.docs.summary Source Review Handoff

This packet prepares the `project.docs.summary` lane for source-level review. It attaches the
...
1. `00_PROJECT_DOCS_SUMMARY_SOURCE_REVIEW_INDEX.md`
2. `01_PROJECT_DOCS_SUMMARY_SOURCE_REVIEW_PROMPT.md`
3. `02_PROJECT_DOCS_SUMMARY_IMPLEMENTATION_PACKET.md`
4. `03_PROJECT_DOCS_SUMMARY_SOURCE_BUNDLE.md`
5. `04_PROJECT_DOCS_SUMMARY_TESTS_BUNDLE.md`
6. `05_PROJECT_DOCS_SUMMARY_CONTRACTS_BUNDLE.md`
7. `06_PROJECT_DOCS_SUMMARY_EVIDENCE.md`
8. `07_PROJECT_DOCS_SUMMARY_FOCUSED_TESTS.txt`
9. `08_PROJECT_DOCS_SUMMARY_INTAKE_COMMANDS.md`
10. `project-docs-summary-source-review-artifact-hashes.json`

This packet does not approve raw recursive listings, raw paths, documentation file names,
documentation headings, file contents, coverage data, command output, dependency names,
package names, package-manager execution, documentation build execution, network access,
broad filesystem access, or public/security-product positioning.
"""


def _prompt(context: dict[str, Any]) -> str:
    return f"""# project.docs.summary Source Review Prompt

You are reviewing Ithildin as a source reviewer for the `project.docs.summary` lane only.
...
resource type `project_docs`, and is free of documentation file names, raw sensitive paths,
file contents, documentation headings, coverage data, command output, package-manager
execution, documentation build execution, registry/network access, shell, and broad
filesystem powers.
"""


def _implementation_packet(context: dict[str, Any]) -> str:
    return f"""# project.docs.summary Implementation Packet

- Tool: `project.docs.summary`
- Commit: `{context["commit"]}`
- Dirty: `{str(context["dirty"]).lower()}`
- Risk/category: `read` / `project`
- Resource type: `project_docs`
- Boundary: documentation-layout counts and allowlisted labels only.
"""


def _evidence(gate_output: str, parity_output: str) -> str:
    return f"""# project.docs.summary Evidence
...
def _intake_commands() -> str:
    return """# project.docs.summary Intake Commands
...
def _bundle_sources(repo_root: Path, paths: list[Path]) -> str:
    ...
    for relative in paths:
        ...
        if not path.exists():
            raise ProjectDocsSummarySourceReviewBundleError(f"missing source: {relative}")
...
def _require_project_root(repo_root: Path) -> None:
    for marker in ("pyproject.toml", "Makefile", "tool-manifests.lock.json"):
        if not (repo_root / marker).exists():
            raise ProjectDocsSummarySourceReviewBundleError("not an Ithildin repository root")
...
def _git(repo_root: Path, args: list[str]) -> str:
    ...
    if process.returncode != 0:
        raise ProjectDocsSummarySourceReviewBundleError(process.stderr.strip())
