"""Build a reviewer-response kit for the planning-only production identity/storage lane."""
...
DEFAULT_OUTPUT_DIR = Path("var/review-packets/v3/production-identity-storage-response-kit")
HASH_MANIFEST = "production-identity-storage-response-kit-artifact-hashes.json"
PROJECT_MARKERS = [
    Path("pyproject.toml"),
    Path("Makefile"),
    Path("apps/api"),
...
class ProductionIdentityStorageResponseKitError(RuntimeError):
    """Raised when the production identity/storage response kit cannot be built."""
...
def main() -> int:
    ...
    except ProductionIdentityStorageResponseKitError as exc:
        print(f"production identity/storage response kit failed: {exc}", file=sys.stderr)
        return 1

    print(f"Built production identity/storage response kit at {output_dir}")
    return 0
...
def build_check_report(repo_root: Path) -> dict[str, Any]:
    ...
    with tempfile.TemporaryDirectory() as tmp:
        output_dir = Path(tmp) / "production-identity-storage-response-kit"
        try:
            build_kit(
                repo_root=repo_root,
    ...
    index = contents.get("00_PRODUCTION_IDENTITY_STORAGE_RESPONSE_KIT_INDEX.md", "")
    guide = contents.get("01_PRODUCTION_IDENTITY_STORAGE_RESPONSE_INTAKE_GUIDE.md", "")
    examples = contents.get("02_PRODUCTION_IDENTITY_STORAGE_NORMALIZED_RESPONSE_EXAMPLES.md", "")
    commands = contents.get("03_PRODUCTION_IDENTITY_STORAGE_CLOSURE_TRIAGE_COMMANDS.md", "")
    boundary = contents.get("04_PRODUCTION_IDENTITY_STORAGE_QUEUE_AND_BOUNDARY_STATUS.md", "")
    evidence = contents.get("05_PRODUCTION_IDENTITY_STORAGE_RESPONSE_KIT_EVIDENCE.md", "")

    for phrase in [
        "Tool count remains `24`",
        "What This Kit Does Not Prove",
        "does not close `ERG-006` or `ERG-007`",
        "does not approve production identity or runtime storage",
    ...
    for phrase in [
        ...
        "Only a later committed triage update may move `ERG-006` or `ERG-007`",
    ]:
    ...
    target = "production-identity-storage-response-kit"
    ...
    doc_rel = "docs/codex/production-identity-storage-response-kit.md"
    ...
    if "Production Identity And Storage Response Kit" not in review_index:
        failures.append(
            "review-docs index is missing production identity/storage response kit entry"
        )
    if f"make {target}" not in readme:
        failures.append("README is missing production identity/storage response kit command")
    for text, source in [
        ...
        if "production-identity-storage-response-kit" not in text:
            failures.append(f"{source} is missing production identity/storage response kit pointer")
...
def build_kit(
    *,
    ...
    if dirty and not allow_dirty:
        raise ProductionIdentityStorageResponseKitError(
            "working tree is dirty; commit before production identity/storage response-kit handoff"
        )
    ...
    files = {
        "00_PRODUCTION_IDENTITY_STORAGE_RESPONSE_KIT_INDEX.md": _index(commit, dirty),
        "01_PRODUCTION_IDENTITY_STORAGE_RESPONSE_INTAKE_GUIDE.md": _intake_guide(),
        "02_PRODUCTION_IDENTITY_STORAGE_NORMALIZED_RESPONSE_EXAMPLES.md": _examples(),
        "03_PRODUCTION_IDENTITY_STORAGE_CLOSURE_TRIAGE_COMMANDS.md": _commands(),
        "04_PRODUCTION_IDENTITY_STORAGE_QUEUE_AND_BOUNDARY_STATUS.md": _docs_bundle(
        ...
        "05_PRODUCTION_IDENTITY_STORAGE_RESPONSE_KIT_EVIDENCE.md": _command_evidence(
            command_reports
        ),
    }
...
def _index(commit: str, dirty: bool) -> str:
    boundary = (
        "This response kit does not prove external review happened, does not close `ERG-006` or "
        "`ERG-007`, does not approve implementation planning, and does not approve production "
        "identity or runtime storage. It does not approve production IAM, enterprise RBAC, "
        "tenant/team authorization runtime behavior, remote admin use, runtime Postgres, database "
        "migrations, backup/restore runtime behavior, retention enforcement, hosted control "
        "plane, custody-grade audit claims, compliance automation, hosted telemetry, remote MCP, "
        "SIEM adapter behavior, sandbox orchestration, local model invocation, trusted-host "
        "promotion, shell/Docker/Kubernetes/browser governed powers, arbitrary HTTP, broad "
        "filesystem writes, plugin SDK behavior, new governed tool powers, or "
        "public/security-product positioning."
    )
    return f"""# Production Identity And Storage Response Kit

Status: response-intake kit for planning-only `ERG-006` and `ERG-007` after external/source
disposition review.
...
1. `01_PRODUCTION_IDENTITY_STORAGE_RESPONSE_INTAKE_GUIDE.md`
2. `02_PRODUCTION_IDENTITY_STORAGE_NORMALIZED_RESPONSE_EXAMPLES.md`
3. `03_PRODUCTION_IDENTITY_STORAGE_CLOSURE_TRIAGE_COMMANDS.md`
4. `04_PRODUCTION_IDENTITY_STORAGE_QUEUE_AND_BOUNDARY_STATUS.md`
5. `05_PRODUCTION_IDENTITY_STORAGE_RESPONSE_KIT_EVIDENCE.md`
6. `production-identity-storage-response-kit-artifact-hashes.json`
...
Only a later committed triage update may move `ERG-006` or `ERG-007`, and only after the closure
...
production IAM, enterprise RBAC, tenant/team authorization runtime behavior, runtime Postgres,
database migrations, backup/restore runtime behavior, retention enforcement, Mission Control
runtime behavior, local model invocation, or sandbox orchestration.
"""
...
update while keeping runtime identity/storage behavior blocked, then run:
...
outcome, critical/high, or attempts to close external review directly, keep `ERG-006` and
`ERG-007` as `planning_only`.
"""
...
def _build_command_reports(repo_root: Path, *, run_commands: bool) -> dict[str, Any]:
    reports: dict[str, Any] = {
        ...
        "disposition_closure_check": (
            production_identity_storage_disposition_closure_check.build_report(repo_root)
        ),
...
def render_check_report(report: dict[str, Any]) -> str:
    lines = [
        "Ithildin production identity/storage response kit check",
        ...
        f"erg_006_erg_007_closed: {str(report['erg_006_erg_007_closed']).lower()}",
    ]
    if report["failures"]:
