"""Build a focused git.show.ref_summary source-review bundle."""
...
DEFAULT_OUTPUT_DIR = Path("var/review-packets/v0.9/git-ref-summary-source-review")
...
SOURCE_FILES = [
    ...
    Path("tool-manifests/git-show-ref-summary.yaml"),
...
CONTRACT_DOCS = [
    Path("docs/codex/v0.9-git-ref-summary-implementation.md"),
    Path("docs/codex/capability-implementation-plans/git-show-ref-summary.md"),
    Path("docs/codex/capability-proposals/git-show-ref-summary.md"),
    Path("docs/codex/v0.9-git-ref-summary-proposal-review.md"),
...
IMPLEMENTATION_GATE_COMMAND = ["make", "git-ref-summary-implementation-gate"]
POLICY_PARITY_COMMAND = ["make", "policy-parity"]


class GitRefSummarySourceReviewBundleError(RuntimeError):
    """Raised when the git ref summary source-review bundle cannot be built."""
...
def main() -> int:
    ...
    except GitRefSummarySourceReviewBundleError as exc:
        print(f"git.show.ref_summary source-review bundle failed: {exc}", file=sys.stderr)
        return 1

    print(f"Built git.show.ref_summary source-review bundle at {output_dir}")
    return 0
...
def build_bundle(
    *,
    ...
    if dirty and not allow_dirty:
        raise GitRefSummarySourceReviewBundleError(
    ...
    files = {
        "00_GIT_REF_SUMMARY_SOURCE_REVIEW_INDEX.md": _index(context),
        "01_GIT_REF_SUMMARY_SOURCE_REVIEW_PROMPT.md": _prompt(context),
        "02_GIT_REF_SUMMARY_IMPLEMENTATION_PACKET.md": implementation_packet,
        "03_GIT_REF_SUMMARY_SOURCE_BUNDLE.md": _bundle_sources(repo_root, SOURCE_FILES),
        "04_GIT_REF_SUMMARY_TESTS_BUNDLE.md": _bundle_sources(repo_root, TEST_FILES),
        "05_GIT_REF_SUMMARY_CONTRACTS_BUNDLE.md": _bundle_sources(repo_root, CONTRACT_DOCS),
        "08_GIT_REF_SUMMARY_INTAKE_COMMANDS.md": _intake_commands(context),
    }
    ...
    (output_dir / "06_GIT_REF_SUMMARY_EVIDENCE.md").write_text(
        _packet_text(_evidence(gate_output, parity_output)),
        encoding="utf-8",
    )
    _write_command_output(
        output_dir / "07_GIT_REF_SUMMARY_FOCUSED_TESTS.txt",
        _command_output(FOCUSED_TEST_COMMAND, run_commands=run_commands),
    )
    _write_json(
        output_dir / "git-ref-summary-source-review-artifact-hashes.json",
        _hashes(output_dir),
    )
    return output_dir
...
def _index(context: dict[str, Any]) -> str:
    return f"""# git.show.ref_summary Source Review Handoff

This packet prepares the `git.show.ref_summary` lane for source-level review. It attaches the
...
1. `00_GIT_REF_SUMMARY_SOURCE_REVIEW_INDEX.md`
2. `01_GIT_REF_SUMMARY_SOURCE_REVIEW_PROMPT.md`
3. `02_GIT_REF_SUMMARY_IMPLEMENTATION_PACKET.md`
4. `03_GIT_REF_SUMMARY_SOURCE_BUNDLE.md`
5. `04_GIT_REF_SUMMARY_TESTS_BUNDLE.md`
6. `05_GIT_REF_SUMMARY_CONTRACTS_BUNDLE.md`
7. `06_GIT_REF_SUMMARY_EVIDENCE.md`
8. `07_GIT_REF_SUMMARY_FOCUSED_TESTS.txt`
9. `08_GIT_REF_SUMMARY_INTAKE_COMMANDS.md`
10. `git-ref-summary-source-review-artifact-hashes.json`

## What This Does Not Prove

This packet does not approve broader capability expansion, arbitrary Git command execution, shell
access, branch/tag mutation, remote fetch, raw ref-name exposure, stable ref-name hashes, raw diffs,
file contents, broad filesystem access, or public/security-product positioning.
"""


def _prompt(context: dict[str, Any]) -> str:
    return f"""# git.show.ref_summary Source Review Prompt

You are reviewing Ithildin as a source reviewer for the `git.show.ref_summary` lane only. Treat this
as source-level review if and only if you inspect the attached source bundle, focused tests, parity
fixture, contract docs, implementation packet, and command evidence.
...
- manifest/schema shape: read risk, Git category, MCP exposure, `selector.kind` enum, `limit`
  bounds, `additionalProperties: false`, and no caller-controlled argv, format strings, ref names,
  refspecs, remotes, pathspecs, diffs, headers, or bodies;
- fixed Git argv behavior for local branches/tags only, with no shell, checkout, mutation, remote
  fetch, reflog access, or raw stderr leakage;
- workspace safety: repository toplevel must be inside the configured workspace for preview and
  runtime;
- output safety: no raw ref names, no stable ref-name hashes, response-local `ref_id` handles only,
  resolved commit hashes, current-branch boolean, tag peeling evidence, count/truncation evidence,
  and untrusted metadata labeling;
- denial/safe failure cases for unsupported selectors, unknown fields, non-commit refs,
  casefold-conflicting refs, control/non-NFC names, parent-repository escape, oversized output, and
...
Please answer whether the `git.show.ref_summary` lane can close for the v0.1 local-preview runtime
...
| --- | --- | --- | --- |
| EXT-GITREF-### | critical/high/medium/low/informational | git-ref-summary | path/function |

Also include blocking status, disposition, and recommended fix for every actionable finding.

Do not approve arbitrary Git execution, raw ref names, stable ref-name hashes, raw diffs, file
...
def _implementation_packet(commit: str, dirty: bool, run_commands: bool) -> str:
    report = _command_output(
        ["uv", "run", "python", "scripts/git_ref_summary_implementation_gate.py", "--json"],
        run_commands=run_commands,
    )
    return "\n".join(
        [
            "# git.show.ref_summary Implementation Packet",
            "",
            f"- Reviewed commit: `{commit}`",
            f"- Dirty at generation: `{str(dirty).lower()}`",
            "- Tool: `git.show.ref_summary`",
            "- Status: approved v0.9 implementation for one bounded read-only Git ref metadata",
            "  tool.",
            "- Boundary: no shell, no caller-controlled Git argv/format/pathspec/refspec, no raw",
            "  ref names, no stable ref-name hashes, no remote refs, no file contents, no raw",
            ...
            "",
            "`git.show.ref_summary` passed xhigh internal source review during implementation",
            "with no blockers. Low-risk findings were remediated by adding safe audit metadata,",
            "mirroring nested schema checks in the no-new-powers guardrail, and documenting that",
            "`ref_id` values are response-local display handles rather than privacy-preserving",
            "randomness.",
        ]
    )
...
def _evidence(gate_output: dict[str, str | int], parity_output: dict[str, str | int]) -> str:
    return "\n".join(
        [
            "# git.show.ref_summary Evidence",
            "",
            "## Implementation Gate",
            "",
            "Command: `make git-ref-summary-implementation-gate`",
...
uv run python scripts/external_response_normalize.py \\
  ...
  --area "git-ref-summary" \\
  --output var/review-runs/v0.9/git-ref-summary/normalized-response.json
```

Use finding IDs in the `EXT-GITREF-###` namespace.
"""
...
def _bundle_sources(repo_root: Path, paths: list[Path]) -> str:
    ...
    for path in paths:
        ...
        if not full_path.exists():
            raise GitRefSummarySourceReviewBundleError(f"missing review file: {path}")
        sections.extend(
            [
                f"# {path.as_posix()}",
                "",
...
def _require_project_root(repo_root: Path) -> None:
    ...
    if not all(path.exists() for path in required):
        raise GitRefSummarySourceReviewBundleError("must be run from the Ithildin repo root")
