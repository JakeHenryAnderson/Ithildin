"""Validate the git.show.tag_metadata implementation-planning packet."""
...
from scripts import git_tag_metadata_proposal_check, v09_design_only_gate

ROOT = Path(__file__).resolve().parents[1]
PLAN_DOC = ROOT / "docs/codex/capability-implementation-plans/git-show-tag-metadata.md"
REQUIRED_PHRASES = [
...
def build_report(repo_root: Path) -> dict[str, Any]:
    ...
    if not plan_path.exists():
        return _report(["git.show.tag_metadata implementation plan is missing"], {})
    ...
    proposal = git_tag_metadata_proposal_check.build_report(repo_root)
...
def render_report(report: dict[str, Any]) -> str:
    lines = [
        "Ithildin git.show.tag_metadata implementation-plan check",
