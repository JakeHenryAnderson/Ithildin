"""Create a focused ERG-004 runtime gate-readiness response inbox."""
...
DOC_NAME = "sandbox-vm-live-poc-runtime-gate-readiness-response-inbox.md"
DEFAULT_OUTPUT_DIR = Path(
    "var/review-runs/sandbox-vm-live-poc-runtime-gate-readiness-response-inbox"
)
INDEX_NAME = "ERG004_RUNTIME_GATE_READINESS_RESPONSE_INBOX.md"
CHEATSHEET_NAME = "ERG004_RUNTIME_GATE_READINESS_CHEATSHEET.md"
JSON_NAME = "erg004-runtime-gate-readiness-response-inbox.json"
HASH_NAME = "erg004-runtime-gate-readiness-response-inbox-artifact-hashes.json"
RAW_NAME = "RAW_RESPONSE_ERG-004-RUNTIME-GATE-READINESS.md"
AREA = "sandbox-vm-live-poc-runtime-gate-readiness"
FINDING_NAMESPACE = "EXT-LIVE-GATE-###"
NORMALIZED_PATH = (
    "var/review-runs/sandbox-vm-live-poc-runtime-gate-readiness/"
...
class SandboxVmLivePocRuntimeGateReadinessResponseInboxError(RuntimeError):
    """Raised when the focused gate-readiness response inbox is invalid."""
...
def main() -> int:
    ...
    except SandboxVmLivePocRuntimeGateReadinessResponseInboxError as exc:
        print(f"ERG-004 runtime gate-readiness response inbox failed: {exc}", file=sys.stderr)
        return 1
    print(f"Built ERG-004 runtime gate-readiness response inbox at {output_dir}")
    return 0
...
def build_inbox(repo_root: Path, output_dir: Path = DEFAULT_OUTPUT_DIR) -> Path:
    ...
    if bundle_report.get("valid") is not True:
        raise SandboxVmLivePocRuntimeGateReadinessResponseInboxError(
            "runtime gate-readiness review bundle is not valid"
        )
    if bundle_report.get("runtime_implementation_allowed") is not False:
        raise SandboxVmLivePocRuntimeGateReadinessResponseInboxError(
            "runtime gate-readiness review unexpectedly allows implementation"
        )
    if bundle_report.get("closes_erg_004") is not False:
        raise SandboxVmLivePocRuntimeGateReadinessResponseInboxError(
            "runtime gate-readiness review unexpectedly closes ERG-004"
        )
...
def build_check_report(repo_root: Path) -> dict[str, Any]:
    ...
    intake_doc = _read(
        repo_root / "docs/codex/sandbox-vm-live-poc-runtime-gate-readiness-response-intake.md"
    )
    dry_run_doc = _read(
        repo_root / "docs/codex/sandbox-vm-live-poc-runtime-gate-readiness-response-dry-run.md"
    )
    ...
    try:
        ...
        payload = {}
        failures.append("ERG-004 runtime gate-readiness response inbox JSON is invalid")
    try:
        hash_manifest = json.loads(hash_text) if hash_text else {"artifacts": []}
    except json.JSONDecodeError:
        hash_manifest = {"artifacts": []}
        failures.append("ERG-004 runtime gate-readiness hash manifest is invalid")
    ...
    if missing_hashes:
        failures.append(
            "ERG-004 runtime gate-readiness inbox hash manifest is missing: "
            + ", ".join(missing_hashes)
        )
    if HASH_NAME in hashed_paths:
        failures.append("ERG-004 runtime gate-readiness hash manifest must not hash itself")
    if not _artifact_hashes_match_files(output_dir, hash_manifest):
        failures.append("ERG-004 runtime gate-readiness artifact hashes do not match")
    ...
    for phrase in [
        ...
        if phrase not in inbox_doc:
            failures.append(f"ERG-004 runtime gate-readiness inbox doc is missing: {phrase}")

    for phrase in [
        "ERG-004 Runtime Gate-Readiness Response Inbox",
        RAW_NAME,
        CHEATSHEET_NAME,
        FINDING_NAMESPACE,
        ...
        if phrase not in index_text:
            failures.append(f"generated ERG-004 runtime gate-readiness inbox is missing: {phrase}")
    for phrase in [
        "ERG-004 Runtime Gate-Readiness Cheat Sheet",
        RAW_NAME,
        ...
        if phrase not in cheatsheet_text:
            failures.append(f"ERG-004 runtime gate-readiness cheat sheet is missing: {phrase}")
    for phrase in [
        '"inbox_type": "ithildin.erg004_runtime_gate_readiness_response_inbox"',
        '"gap": "ERG-004"',
        ...
        if phrase not in json_text:
            failures.append(f"ERG-004 runtime gate-readiness JSON is missing: {phrase}")
    for phrase in [
        "Paste the real EXT-LIVE-GATE reviewer response here.",
        "Finding namespace: EXT-LIVE-GATE-###",
        "Do not paste secrets, keys, prompts, model outputs, or file contents",
    ]:
        if phrase not in raw_text:
            failures.append(f"ERG-004 runtime gate-readiness raw placeholder is missing: {phrase}")

    if payload.get("gap") != "ERG-004":
        failures.append("ERG-004 runtime gate-readiness payload gap drifted")
    if payload.get("normalization_area") != AREA:
        failures.append("ERG-004 runtime gate-readiness normalization area drifted")
    if payload.get("finding_namespace") != FINDING_NAMESPACE:
        failures.append("ERG-004 runtime gate-readiness finding namespace drifted")
    if not str(payload.get("reviewed_packet_hash", "")).startswith("sha256:"):
        failures.append("ERG-004 runtime gate-readiness packet hash is missing")
    if payload.get("runtime_implementation_allowed") is not False:
        failures.append("ERG-004 runtime gate-readiness payload allows implementation")

    if "sandbox-vm-live-poc-runtime-gate-readiness-response-inbox:" not in makefile:
        failures.append("Make target is missing: runtime gate-readiness response inbox")
    if "sandbox-vm-live-poc-runtime-gate-readiness-response-inbox-check:" not in makefile:
        failures.append("Make target is missing: runtime gate-readiness response inbox check")
    if (
        ...
        failures.append("runtime gate-readiness response inbox check missing from release-check")
    if "$(MAKE) sandbox-vm-live-poc-runtime-gate-readiness-response-inbox" not in (
        review_candidate_body
    ):
        failures.append("runtime gate-readiness response inbox missing from review-candidate")
    if "make sandbox-vm-live-poc-runtime-gate-readiness-response-inbox" not in readme:
        failures.append("README is missing runtime gate-readiness response inbox command")
    if DOC_REL not in readme:
        failures.append("README is missing runtime gate-readiness response inbox doc")
    if DOC_REL not in docs_site:
        failures.append("runtime gate-readiness response inbox missing from docs site")
    if DOC_REL not in review_docs.REVIEW_DOCS:
        failures.append("runtime gate-readiness response inbox missing from review docs")
    if DOC_NAME not in review_index:
        failures.append("review docs index missing runtime gate-readiness response inbox")
    if "sandbox-vm-live-poc-runtime-gate-readiness-response-inbox-check" not in (
        release_guardrails
    ):
        failures.append("release guardrails missing runtime gate-readiness response inbox")
    for label, content in [
        ("response intake", intake_doc),
        ("response dry run", dry_run_doc),
        ("enterprise response command matrix", matrix_doc),
    ]:
        if "sandbox-vm-live-poc-runtime-gate-readiness-response-inbox" not in content:
            failures.append(f"{label} missing runtime gate-readiness response inbox pointer")
...
def render_check_report(report: dict[str, Any]) -> str:
    lines = [
        "Ithildin ERG-004 runtime gate-readiness response inbox",
...
def _payload(repo_root: Path, output_dir: Path) -> dict[str, Any]:
    ...
    return {
        ...
        "gap": "ERG-004",
        "status": "ready_for_runtime_implementation_gate_review",
        "output_dir": output_dir.as_posix(),
        ...
        "dry_run": "make sandbox-vm-live-poc-runtime-gate-readiness-response-dry-run",
        "decision_skeleton_check": (
            "make sandbox-vm-live-poc-runtime-gate-readiness-decision-record-skeleton-check"
        ),
...
def _render_index(payload: dict[str, Any]) -> str:
    return f"""# ERG-004 Runtime Gate-Readiness Response Inbox

Status: generated response inbox for the active ERG-004 runtime gate-readiness review.

This inbox gives the active `EXT-LIVE-GATE-###` review path an exact raw-response placeholder,
...
`ERG-004`, and does not approve runtime implementation.
...
def _render_cheatsheet(payload: dict[str, Any]) -> str:
    return f"""# ERG-004 Runtime Gate-Readiness Cheat Sheet

Paste a real `EXT-LIVE-GATE-###` reviewer response into:
...
def _render_raw_placeholder(payload: dict[str, Any]) -> str:
    return f"""# Raw ERG-004 Runtime Gate-Readiness Reviewer Response
