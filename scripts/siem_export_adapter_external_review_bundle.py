"""Build a reviewer-friendly ERG-008 SIEM export adapter bundle."""
...
DEFAULT_OUTPUT_DIR = Path("var/review-packets/v3/siem-export-adapter-external-review")
HASH_MANIFEST = "siem-export-adapter-external-review-artifact-hashes.json"
PROJECT_MARKERS = [
    Path("pyproject.toml"),
    Path("Makefile"),
    Path("apps/api"),
...
class SiemExportAdapterExternalReviewBundleError(RuntimeError):
...
def main() -> int:
    ...
    except SiemExportAdapterExternalReviewBundleError as exc:
        print(f"SIEM export adapter external-review bundle failed: {exc}", file=sys.stderr)
        return 1

    print(f"Built SIEM export adapter external-review bundle at {output_dir}")
    return 0
...
def build_check_report(repo_root: Path) -> dict[str, Any]:
    ...
    with tempfile.TemporaryDirectory() as tmp:
        output_dir = Path(tmp) / "siem-export-adapter-external-review"
        try:
            build_bundle(
                repo_root=repo_root,
    ...
    index = contents.get("00_SIEM_EXPORT_ADAPTER_EXTERNAL_REVIEW_INDEX.md", "")
    prompt = contents.get("01_SIEM_EXPORT_ADAPTER_EXTERNAL_REVIEW_PROMPT.md", "")
    response = contents.get("04_SIEM_EXPORT_ADAPTER_RESPONSE_CLOSURE_DRY_RUN.md", "")
    reproduction = contents.get("05_SIEM_EXPORT_ADAPTER_REPRODUCTION_QUEUE_STATUS.md", "")
    evidence = contents.get("07_SIEM_EXPORT_ADAPTER_COMMAND_EVIDENCE.md", "")

    for phrase in [
        "Finding namespace: `EXT-SIEM-ADAPTER-###`",
        "Can `ERG-008` continue architecture planning",
        "Do not approve SIEM adapter runtime behavior",
        "Do not approve hosted telemetry",
    ]:
    ...
    target = "siem-export-adapter-external-review-bundle"
    ...
    doc_rel = "docs/codex/siem-export-adapter-external-review-bundle.md"
    ...
    if "SIEM Export Adapter External Review Bundle" not in review_index:
    ...
    for text, source in [
        ...
        if "siem-export-adapter-external-review-bundle" not in text:
...
def build_bundle(
    *,
    ...
    if dirty and not allow_dirty:
        raise SiemExportAdapterExternalReviewBundleError(
    ...
    with tempfile.TemporaryDirectory() as tmp:
        ...
        files = {
            "00_SIEM_EXPORT_ADAPTER_EXTERNAL_REVIEW_INDEX.md": _index(commit, dirty),
            "01_SIEM_EXPORT_ADAPTER_EXTERNAL_REVIEW_PROMPT.md": _prompt(commit),
            "02_SIEM_EXPORT_ADAPTER_DISPOSITION_PACKET.md": _packet_bundle(
                "Disposition Packet Contents", disposition_dir
            ),
            "03_SIEM_EXPORT_ADAPTER_ARCHITECTURE_CONTRACTS.md": _docs_bundle(
            ...
            "04_SIEM_EXPORT_ADAPTER_RESPONSE_CLOSURE_DRY_RUN.md": _docs_bundle(
            ...
            "05_SIEM_EXPORT_ADAPTER_REPRODUCTION_QUEUE_STATUS.md": _docs_bundle(
            ...
            "06_SIEM_EXPORT_ADAPTER_BOUNDARY_EVIDENCE.md": _docs_bundle(
            ...
            "07_SIEM_EXPORT_ADAPTER_COMMAND_EVIDENCE.md": _command_evidence(command_reports),
        }
...
def _index(commit: str, dirty: bool) -> str:
    return f"""# SIEM Export Adapter External Review Bundle

Status: reviewer launch bundle for `ERG-008`.
...
Recommended next review: `ERG-008` SIEM-shaped export adapter architecture disposition.

## Reading Order

1. `01_SIEM_EXPORT_ADAPTER_EXTERNAL_REVIEW_PROMPT.md`
2. `02_SIEM_EXPORT_ADAPTER_DISPOSITION_PACKET.md`
3. `03_SIEM_EXPORT_ADAPTER_ARCHITECTURE_CONTRACTS.md`
4. `04_SIEM_EXPORT_ADAPTER_RESPONSE_CLOSURE_DRY_RUN.md`
5. `05_SIEM_EXPORT_ADAPTER_REPRODUCTION_QUEUE_STATUS.md`
6. `06_SIEM_EXPORT_ADAPTER_BOUNDARY_EVIDENCE.md`
7. `07_SIEM_EXPORT_ADAPTER_COMMAND_EVIDENCE.md`
8. `siem-export-adapter-external-review-artifact-hashes.json`

## What This Bundle Does Not Prove

This bundle does not prove that external review has happened, does not close `ERG-008`, and does not
approve SIEM adapter runtime behavior, hosted telemetry, remote delivery, custody-grade audit
claims, external notarization, immutable storage, production identity, runtime Postgres, compliance
automation, security-operations control-plane claims, hosted control plane, remote MCP, sandbox
...
This bundle does not approve SIEM adapter runtime behavior.
"""


def _prompt(commit: str) -> str:
    return f"""# SIEM Export Adapter External Review Prompt

You are reviewing Ithildin as an external/source reviewer for `ERG-008` only: the SIEM-shaped export
adapter architecture lane.
...
Review the attached disposition packet, SIEM-shaped evidence design, architecture contract, evidence
contracts, decision-register pointers, external response intake, closure gate, dry-run evidence,
queue status, and command evidence.

Please answer:

1. Did you inspect the SIEM export adapter disposition packet and architecture packet?
2. Are the event schema, category allowlist, required fields, compatibility policy, redaction
   denylist, and no-export behavior complete enough for continued architecture planning?
3. Are the delivery profile, retry, dead-letter, backpressure, destination-authentication,
   signing/verification, idempotency/replay, and diagnostics questions complete enough for continued
   architecture planning?
4. Are the secret-free evidence fields safe and useful for future offline export and incident
   reconstruction?
...
7. Can `ERG-008` continue architecture planning while runtime implementation remains blocked?

Do not approve SIEM adapter runtime behavior. Do not approve hosted telemetry. Do not approve remote
delivery. Do not approve custody-grade audit claims. Do not approve external notarization. Do not
approve immutable storage. Do not approve production identity. Do not approve runtime Postgres. Do
not approve compliance automation. Do not approve security-operations control-plane claims. Do not
approve sandbox orchestration. Do not approve trusted-host promotion. Do not approve
public/security-product positioning.

Use this finding namespace for actionable findings: `EXT-SIEM-ADAPTER-###`.
...
def _build_command_reports(repo_root: Path, *, run_commands: bool) -> dict[str, Any]:
    reports: dict[str, Any] = {
        "disposition_packet_check": siem_export_adapter_disposition_packet.build_check_report(
            repo_root
        ),
        "external_response_intake_check": (
            siem_export_adapter_external_response_intake_check.build_report(repo_root)
        ),
        "closure_gate_check": siem_export_adapter_disposition_closure_check.build_report(
            repo_root
        ),
...
def render_check_report(report: dict[str, Any]) -> str:
    lines = [
        "Ithildin SIEM export adapter external-review bundle check",
