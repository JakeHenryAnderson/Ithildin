"""Validate the approved git.show.ref_summary implementation boundary."""
...
IMPLEMENTATION_DOC = ROOT / "docs/codex/v0.9-git-ref-summary-implementation.md"
MANIFEST_PATH = ROOT / "tool-manifests/git-show-ref-summary.yaml"
...
def build_report(repo_root: Path) -> dict[str, Any]:
    ...
    if not doc_path.exists():
        failures.append("v0.9 git ref summary implementation doc is missing")
    else:
    ...
    if not manifest_path.exists():
        failures.append("git.show.ref_summary manifest is missing")
...
def _validate_manifest(manifest: dict[str, Any]) -> list[str]:
    failures: list[str] = []
    if manifest.get("name") != "git.show.ref_summary":
...
def render_report(report: dict[str, Any]) -> str:
    lines = [
        "Ithildin git.show.ref_summary implementation gate",
