"""Validate ERG-004 runtime gate-readiness response-application playbook wiring."""
...
DOC_NAME = "sandbox-vm-live-poc-runtime-gate-readiness-response-application-playbook.md"
...
def build_report(repo_root: Path) -> dict[str, Any]:
    ...
    if not doc:
        failures.append("ERG-004 gate-readiness response application playbook doc is missing")
    else:
        ...
        for boundary in REQUIRED_BLOCKED_BOUNDARIES:
            if boundary not in doc:
                failures.append(
                    "response application playbook is missing blocked boundary: "
                    f"{boundary}"
                )
    ...
    target = "sandbox-vm-live-poc-runtime-gate-readiness-response-application-playbook-check"
