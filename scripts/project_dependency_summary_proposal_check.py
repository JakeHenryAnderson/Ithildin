"""Validate the project.dependency.summary design-only capability proposal."""
...
SELECTION_DOC = ROOT / "docs/codex/v3-project-dependency-summary-selection.md"
REQUIRED_PHRASES = [
...
def build_report(repo_root: Path) -> dict[str, Any]:
    ...
    if not proposal_path.exists():
        return _report(["project.dependency.summary proposal is missing"], {})
    ...
    if not selection_path.exists():
        failures.append("project.dependency.summary selection doc is missing")
    else:
...
def render_report(report: dict[str, Any]) -> str:
    lines = [
        "Ithildin project.dependency.summary proposal check",
