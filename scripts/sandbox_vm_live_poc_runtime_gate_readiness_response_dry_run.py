"""Exercise ERG-004 runtime gate-readiness response intake without recording review."""
...
DOC_NAME = "sandbox-vm-live-poc-runtime-gate-readiness-response-dry-run.md"
NORMALIZED_RESPONSE_REL = (
    "var/review-runs/sandbox-vm-live-poc-runtime-gate-readiness/normalized-response.json"
)
EXPECTED_OUTCOME = "approved_for_descriptor_only_runtime_implementation_planning"
...
def run_dry_run(repo_root: Path) -> dict[str, Any]:
    ...
    with response_dry_run_lock(repo_root, NORMALIZED_RESPONSE_REL):
        ...
        try:
            ...
            cases["absent_response_keeps_planning_blocked"] = (
                absent_report["descriptor_only_planning_allowed"] is False
                and absent_report["runtime_implementation_allowed"] is False
            )
            ...
            cases["favorable_response_shape_intakes"] = (
                favorable["finding_count"] == 0
                and favorable["can_close_source_rows"] is True
                and _decision_record_consideration_ready(favorable, favorable_text) is True
            )
            ...
            cases["packet_only_response_not_decision_ready"] = (
                packet_only["can_close_source_rows"] is False
                and _decision_record_consideration_ready(packet_only, packet_only_text) is False
            )
            ...
            cases["docs_only_response_not_decision_ready"] = (
                docs_only["can_close_source_rows"] is False
                and _decision_record_consideration_ready(docs_only, docs_only_text) is False
            )

            missing_outcome_text = _response_text(no_findings=True)
            missing_outcome = _normalize_and_write(normalized_path, missing_outcome_text)
            cases["missing_outcome_not_decision_ready"] = (
                missing_outcome["finding_count"] == 0
                and _decision_record_consideration_ready(
            ...
            cases["critical_high_finding_not_decision_ready"] = (
                high_finding["finding_count"] == 1
                and _has_critical_high_findings(high_finding) is True
                and _decision_record_consideration_ready(high_finding, high_finding_text)
                is False
            )
            ...
            cases["missing_explicit_finding_statement_rejected"] = _normalization_rejected(
                "Overall judgment: looks fine.\n"
            )
        finally:
            if original is None:
                normalized_path.unlink(missing_ok=True)
...
def _decision_record_consideration_ready(response: dict[str, Any], text: str) -> bool:
    return (
...
def _response_text(
    *,
    ...
    lines = [
        "Overall judgment: fixture response for runtime gate-readiness intake.",
...
def render_report(report: dict[str, Any]) -> str:
    lines = [
        "Ithildin sandbox/VM live POC runtime gate-readiness response dry run",
