"""Build the project.test.summary design-review packet."""
...
DEFAULT_OUTPUT_DIR = Path("var/review-packets/v3/project-test-summary-design-review")
...
DOCS = {
    ...
    "03_PROJECT_TEST_SUMMARY_SELECTION.md": Path("docs/codex/v3-project-test-summary-selection.md"),
    "04_PROJECT_TEST_SUMMARY_PROPOSAL.md": Path(
...
class ProjectTestSummaryDesignReviewPacketError(RuntimeError):
...
def main() -> int:
    ...
    except ProjectTestSummaryDesignReviewPacketError as exc:
        print(f"project.test.summary design-review packet failed: {exc}", file=sys.stderr)
        return 1

    print(f"Built project.test.summary design-review packet at {output_dir}")
    return 0
...
def build_packet(*, repo_root: Path, output_dir: Path, allow_dirty: bool = False) -> Path:
    ...
    if dirty and not allow_dirty:
        raise ProjectTestSummaryDesignReviewPacketError(
    ...
    if failures:
        raise ProjectTestSummaryDesignReviewPacketError("; ".join(failures))
    ...
    files = {
        "00_PROJECT_TEST_SUMMARY_DESIGN_REVIEW_INDEX.md": _index(context),
        "01_PROJECT_TEST_SUMMARY_DESIGN_REVIEW_PROMPT.md": _prompt(context),
    ...
    _write_json(
        output_dir / "project-test-summary-design-review-artifact-hashes.json",
        _hashes(output_dir),
    )
    return output_dir
...
def _index(context: dict[str, Any]) -> str:
    return f"""# project.test.summary Design Review Packet
...
- Capability: `project.test.summary`.
...
1. `00_PROJECT_TEST_SUMMARY_DESIGN_REVIEW_INDEX.md`
2. `01_PROJECT_TEST_SUMMARY_DESIGN_REVIEW_PROMPT.md`
3. `02_NEXT_CAPABILITY_READINESS.md`
4. `03_PROJECT_TEST_SUMMARY_SELECTION.md`
5. `04_PROJECT_TEST_SUMMARY_PROPOSAL.md`
...
10. `project-test-summary-design-review-artifact-hashes.json`

## What This Packet Does Not Prove

This packet does not add a manifest, executor, policy rule, MCP exposure, API behavior, UI
behavior, runtime behavior, test execution, test file names, file-content access, package-manager
execution, registry/network access, coverage claims, or broad governed tool powers.
"""


def _prompt(context: dict[str, Any]) -> str:
    return f"""# project.test.summary Design Review Prompt
...
Reviewed commit: `{context["commit"]}`
Capability: `project.test.summary`
Finding namespace: `EXT-DESIGN-PTS-###`

Please review:

- whether the proposal stays design-only and avoids manifest/executor/policy/MCP/runtime changes;
- whether count-only test metadata and allowlisted labels are useful enough without test file
  names, raw paths, file contents, test names, scripts, coverage, pass/fail state, or command
  output;
...
Use finding IDs `EXT-DESIGN-PTS-###` for actionable findings.
"""


def _gate_evidence(context: dict[str, Any]) -> str:
    return f"""# project.test.summary Gate And Risk Evidence
...
def _intake() -> str:
    return """# project.test.summary Intake And Next Steps

If design review records findings, intake them with finding IDs `EXT-DESIGN-PTS-###` before any
...
def _git(repo_root: Path, args: list[str]) -> str:
    ...
    if process.returncode != 0:
        raise ProjectTestSummaryDesignReviewPacketError(process.stderr.strip())
