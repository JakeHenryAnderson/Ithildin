...
enterprise-e2-scale-fixture-check:
	uv run python scripts/enterprise_e2_scale_fixture.py check
	uv run pytest tests/test_enterprise_e2_scale_fixture.py -q

enterprise-e2-preparation-check:
	uv run python scripts/enterprise_e2_preparation_check.py
	uv run pytest \
...
enterprise-e1-single-site-deployment-check:
	...
	uv run pytest tests/test_enterprise_e1_single_site.py tests/test_enterprise_e1_single_site_rehearsal.py -q
	uv run ruff check scripts/enterprise_e1_single_site.py scripts/enterprise_e1_single_site_deployment_check.py scripts/enterprise_e1_single_site_rehearsal.py tests/test_enterprise_e1_single_site.py tests/test_enterprise_e1_single_site_rehearsal.py
	uv run mypy --strict scripts/enterprise_e1_single_site.py scripts/enterprise_e1_single_site_deployment_check.py scripts/enterprise_e1_single_site_rehearsal.py

enterprise-e1-single-site-rehearsal:
	uv run python scripts/enterprise_e1_single_site_rehearsal.py

enterprise-e1-configuration-check:
	uv run python scripts/enterprise_e1_configuration_check.py
	uv run pytest \
...
enterprise-e1-recovery-check:
	uv run python scripts/enterprise_e1_recovery_check.py
	uv run pytest \
		tests/test_enterprise_e1_recovery.py \
		...
		-q
	uv run ruff check scripts/enterprise_e1_recovery_check.py tests/test_enterprise_e1_recovery.py
	uv run mypy --strict scripts/enterprise_e1_recovery_check.py

enterprise-e1-cockpit-check:
	uv run python scripts/enterprise_e1_cockpit_check.py
	uv run pytest tests/test_enterprise_e1_cockpit.py -q
	uv run ruff check scripts/enterprise_e1_cockpit_check.py tests/test_enterprise_e1_cockpit.py
	uv run mypy --strict scripts/enterprise_e1_cockpit_check.py
	npm run typecheck --prefix apps/ui
	npm run test --prefix apps/ui
	npm run build --prefix apps/ui

enterprise-e1-evidence-check:
	uv run python scripts/enterprise_e1_evidence_check.py
	uv run pytest \
		tests/test_enterprise_e1_evidence.py \
		tests/test_enterprise_e1_operations_bundle.py \
		tests/test_redaction.py \
		-q
	uv run ruff check \
		scripts/enterprise_e1_evidence_check.py \
		scripts/enterprise_e1_operations_bundle.py \
		tests/test_enterprise_e1_evidence.py \
		tests/test_enterprise_e1_operations_bundle.py
	uv run mypy --strict \
		scripts/enterprise_e1_evidence_check.py \
...
enterprise-e1-candidate-preflight:
	@test -n "$(E1_CANDIDATE_COMMIT)" || (echo "E1_CANDIDATE_COMMIT is required" >&2; exit 2)
...
enterprise-e1-candidate-check:
	$(MAKE) enterprise-e1-candidate-preflight E1_CANDIDATE_COMMIT="$(E1_CANDIDATE_COMMIT)"
	$(MAKE) enterprise-e1-candidate-inventory
	$(MAKE) enterprise-e1-candidate-preflight E1_CANDIDATE_COMMIT="$(E1_CANDIDATE_COMMIT)"
	@echo "candidate_commit=$$(git rev-parse HEAD)"
	@echo "candidate_tree=$$(git rev-parse HEAD^{tree})"
	@echo "candidate_tree_clean=true"
	@echo "enterprise_e1_candidate_gate_returncode=0"
...
# LIVE, recovery-only, one-shot operator entrypoint. Never part of aggregate checks.
local-v1-lv1-003-o4-image-recovery-run:
	uv run python -m scripts.local_v1_lv1_003_o4_image_recovery

# Fake-only static gate. It performs no Docker, process, provider, credential, or live action.
local-v1-lv1-003-o4-attempt008-port-recovery-check:
	uv run pytest tests/test_local_v1_lv1_003_o4_attempt008_port_release.py -q

# LIVE, exact-project, one-shot recovery entrypoint. Never part of aggregate checks.
local-v1-lv1-003-o4-attempt008-port-release-run:
	uv run python -m scripts.local_v1_lv1_003_o4_attempt008_port_release

# Fixture-only static gate. It reads no retained runtime, receipts, credentials, or live state.
local-v1-lv1-003-o4-attempt008-node-identity-reconciliation-check:
	uv run pytest tests/test_local_v1_lv1_003_o4_attempt008_node_identity_reconciliation.py -q

# LIVE, exact-database, read-only, one-shot projection. Never part of aggregate checks.
local-v1-lv1-003-o4-attempt008-node-identity-reconciliation-run:
	uv run python -m scripts.local_v1_lv1_003_o4_attempt008_node_identity_reconciliation

# Fixture-only recovery gate. It performs no Docker, runtime, receipt, or live storage action.
local-v1-lv1-003-o4-attempt008-quarantine-revocation-check:
	uv run pytest tests/test_local_v1_lv1_003_o4_attempt008_quarantine_revocation.py -q

# LIVE, exact-project, resumable one-shot recovery entrypoint. Never part of aggregate checks.
local-v1-lv1-003-o4-attempt008-quarantine-revocation-run:
	uv run --offline --frozen python -m scripts.local_v1_lv1_003_o4_attempt008_quarantine_revocation

# Fixture-only successor gate. It performs no Docker, runtime, receipt, or live storage action.
local-v1-lv1-003-o4-attempt008-two-container-revocation-check:
	uv run pytest tests/test_local_v1_lv1_003_o4_attempt008_two_container_revocation.py -q

# LIVE, observation-only Docker precondition plus offline resumable revocation. Never aggregated.
local-v1-lv1-003-o4-attempt008-two-container-revocation-run:
	uv run --offline --frozen python -m scripts.local_v1_lv1_003_o4_attempt008_two_container_revocation
...
mission-command-runner-bridge-implementation-check:
	uv run pytest \
...
local-v1-constrained-mission-contract-check:
	uv run pytest tests/test_local_v1_constrained_mission_journey.py -q
...
local-v1-typecheck:
	...
	uv run mypy --strict \
		...
		scripts/local_v1_node_journey.py \
		scripts/local_v1_node_journey_evidence.py \
		scripts/local_v1_node_journey_check.py \
		scripts/local_v1_constrained_mission_journey.py \
		scripts/local_v1_constrained_mission_journey_check.py \
		scripts/local_v1_failure_recovery_check.py \
	...
	uv run mypy --strict \
		scripts/mission_command_control_plane_plan_check.py \
		scripts/mission_command_control_plane_poc_evidence_check.py \
...
local-v1-real-agent-static-check:
	...
	uv run ruff check scripts/local_v1_real_agent_rehearsal.py scripts/local_v1_real_agent_rehearsal_check.py tests/test_local_v1_real_agent_rehearsal.py
	uv run mypy --strict scripts/local_v1_real_agent_rehearsal.py scripts/local_v1_real_agent_rehearsal_check.py

local-v1-real-agent-run:
	uv run python -m scripts.local_v1_real_agent_rehearsal

local-v1-real-agent-check:
	@test -n "$(LOCAL_V1_REAL_AGENT_REPORT)" || (echo "LOCAL_V1_REAL_AGENT_REPORT is required" >&2; exit 2)
	@test -n "$(LOCAL_V1_REAL_AGENT_CANDIDATE)" || (echo "LOCAL_V1_REAL_AGENT_CANDIDATE is required" >&2; exit 2)
	uv run python -m scripts.local_v1_real_agent_rehearsal_check \
		--report "$(LOCAL_V1_REAL_AGENT_REPORT)" \
		--expected-candidate "$(LOCAL_V1_REAL_AGENT_CANDIDATE)"
...
release-check: mission-command-runner-bridge-authorization-check
...
# Compatibility name retained after the authoritative release-check definition so substring-based
# release-wiring validators cannot mistake this focused recovery gate for the release gate.
local-v1-lv1-003-o4-attempt008-port-release-check: local-v1-lv1-003-o4-attempt008-port-recovery-check
