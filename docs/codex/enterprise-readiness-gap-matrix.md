...
| `ERG-005` | Trusted-host artifact promotion | `staging_only_runtime_source_review_ready` | Moving one approved artifact into host staging | Artifact hash-binding model, approval model, the trusted host descriptor contract in `trusted-host-descriptor-contract.md`, the Trusted-Host Promotion Decision Intake in `trusted-host-promotion-decision-intake.md`, the state machine in `trusted-host-promotion-state-machine.md`, the negative fixture contract in `trusted-host-promotion-negative-fixtures.md`, the zone contract in `trusted-host-promotion-zone-contract.md`, the implementation-plan contract in `trusted-host-promotion-implementation-plan.md`, the source-review handoff in `trusted-host-promotion-source-review.md`, the disposition packet in `trusted-host-promotion-disposition-packet.md`, the launch bundle in `trusted-host-promotion-external-review-bundle.md`, the response kit in `trusted-host-promotion-response-kit.md`, the external response intake template in `trusted-host-promotion-external-response-intake.md`, the fail-closed closure gate in `trusted-host-promotion-disposition-closure-gate.md`, the response dry-run fixture checker in `trusted-host-promotion-response-dry-run.md`, the decision record in `trusted-host-promotion-decision-record.md`, the Goal C decision in `trusted-host-promotion-implementation-gate-decision.md`, the runtime implementation decision in `trusted-host-promotion-runtime-implementation-decision.md`, the runtime implementation record in `trusted-host-promotion-runtime-implementation.md`, the internal runtime review in `v3-trusted-host-promotion-runtime-internal-review.md`, the closure addendum in `v3-trusted-host-promotion-runtime-review-closure.md`, observed negative transcripts, and the focused runtime source-review bundle in `trusted-host-promotion-runtime-source-review.md`; broad trusted-host promotion, approved-output publishing, arbitrary host writes, automatic promotion, Ithildin Command Center runtime authority, and new powers remain blocked |
| `ERG-006` | Production identity and multi-user authorization | `planning_only` | Enterprise identity, RBAC, tenant/team authorization, and remote admin use | The reviewed `production-identity-storage-architecture.md`, committed `production-identity-storage-architecture-decision-record.md`, bounded `production-identity-storage-pis-001-planning-gate.md`, and later reviewed PIS-003 environment-evidence authority remain the historical lineage. The current authority requires external operator target identity and signed receipt inputs before any separately reviewed collection-action authority can be prepared. Historical review lineage remains in `production-identity-storage-disposition-packet.md`, `production-identity-storage-external-review-bundle.md`, `production-identity-storage-response-kit.md`, `production-identity-storage-response-dry-run.md`, and `production-identity-storage-external-response-intake.md`. The wait state does not authorize target selection by the repository, receipt collection, dependency changes, production identity, enterprise RBAC, remote administration, or runtime work. The closed authority record and `production-identity-storage-disposition-closure-gate.md` remain governing evidence. |
| `ERG-007` | Durable runtime storage and retention | `planning_only` | Runtime Postgres, multi-user concurrency, retention policy, backup/restore, and production custody | The reviewed `production-identity-storage-architecture.md`, committed `production-identity-storage-architecture-decision-record.md`, bounded `production-identity-storage-pis-001-planning-gate.md`, and later reviewed PIS-003 environment-evidence authority remain the historical lineage. The current authority requires external operator target identity and signed receipt inputs before any separately reviewed collection-action authority can be prepared. The wait state does not authorize target selection by the repository, receipt collection, dependencies, schemas, migrations, runtime PostgreSQL, backup/restore behavior, retention enforcement, or production custody. The closed authority record, `production-identity-storage-disposition-closure-gate.md`, and database security profile remain governing evidence. |
...
| `ERG-009` | Compliance mapping support | `planning_only` | HIPAA/GLBA/SOX/GDPR or other compliance claims | The design-only architecture packet in `compliance-mapping-architecture.md`, the static synthetic template corpus in `compliance-mapping-template-compatibility-fixtures.md`, the disposition packet in `compliance-mapping-disposition-packet.md`, the launch bundle in `compliance-mapping-external-review-bundle.md`, the response kit in `compliance-mapping-response-kit.md`, the external response intake template in `compliance-mapping-external-response-intake.md`, the fail-closed closure gate in `compliance-mapping-disposition-closure-gate.md`, the response dry-run fixture checker in `compliance-mapping-response-dry-run.md`, real-framework selection and versioning, legal-review boundary, operator responsibility language, evidence reconstruction guide, explicit no-compliance-automation wording, post-RC decision record, and external/source review |
...
administration, runtime Postgres, migrations, response evidence, closure, or runtime expansion.
