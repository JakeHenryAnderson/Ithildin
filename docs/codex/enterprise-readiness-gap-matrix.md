...
| `ERG-006` | Production identity and multi-user authorization | `planning_only` | Enterprise identity, RBAC, tenant/team authorization, and remote admin use | The design-only architecture packet in `production-identity-storage-architecture.md`, exact-candidate source review, committed `production-identity-storage-architecture-decision-record.md`, and bounded `production-identity-storage-pis-001-planning-gate.md` permit only the PIS-001 threat-model, non-goal, dependency-evaluation, exact-contract, and negative-test planning artifact. Historical review lineage remains in `production-identity-storage-disposition-packet.md`, `production-identity-storage-external-review-bundle.md`, `production-identity-storage-response-kit.md`, `production-identity-storage-response-dry-run.md`, and `production-identity-storage-external-response-intake.md`. They do not authorize dependency changes, PIS-002, implementation planning, production identity, enterprise RBAC, remote administration, or runtime work. The fail-closed `production-identity-storage-disposition-closure-gate.md` and a later PIS-002 entry decision remain governing evidence. |
| `ERG-007` | Durable runtime storage and retention | `planning_only` | Runtime Postgres, multi-user concurrency, retention policy, backup/restore, and production custody | The design-only architecture packet in `production-identity-storage-architecture.md`, exact-candidate source review, committed `production-identity-storage-architecture-decision-record.md`, and bounded `production-identity-storage-pis-001-planning-gate.md` permit only the PIS-001 threat-model, dependency-evaluation, exact-contract, and negative-test planning artifact. Historical review lineage remains in `production-identity-storage-disposition-packet.md`, `production-identity-storage-external-review-bundle.md`, `production-identity-storage-response-kit.md`, and `production-identity-storage-external-response-intake.md`. They do not authorize dependency changes, PIS-002, schemas, migrations, runtime PostgreSQL, backup/restore behavior, retention enforcement, or production custody. The fail-closed `production-identity-storage-disposition-closure-gate.md`, database security profile, and a later PIS-002 entry decision remain governing evidence. |
...
Current active route: bounded `PIS-001` threat-model and dependency-decision planning under the recorded `ERG-006`/`ERG-007` architecture decision.

Current expected action:
`execute_pis_001_threat_model_dependency_decision`.

The operator should produce the PIS-001 planning artifact defined by the committed planning gate.
...
not approve dependency changes, PIS-002, production identity, enterprise RBAC, remote
