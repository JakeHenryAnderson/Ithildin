...
| `ERG-006` | Production identity and multi-user authorization | `planning_only` | Enterprise identity, RBAC, tenant/team authorization, and remote admin use | The design-only architecture packet in `production-identity-storage-architecture.md`, the disposition packet in `production-identity-storage-disposition-packet.md`, and the source review plus exact-candidate remediation re-review in `production-identity-storage-source-review.md`; the re-review verified all five medium findings fixed and made the lane eligible for a later post-RC architecture decision record, but did not authorize that record, `PIS-001`, implementation planning, or runtime work. Identity provider design, local-principal mapping, tenant/workspace model, session/admin model, audit attribution model, the launch bundle in `production-identity-storage-external-review-bundle.md`, the response kit in `production-identity-storage-response-kit.md`, the external response intake template in `production-identity-storage-external-response-intake.md`, the fail-closed closure gate in `production-identity-storage-disposition-closure-gate.md`, and the response dry-run fixture checker in `production-identity-storage-response-dry-run.md` remain the governing evidence. |
| `ERG-007` | Durable runtime storage and retention | `planning_only` | Runtime Postgres, multi-user concurrency, retention policy, backup/restore, and production custody | The design-only architecture packet in `production-identity-storage-architecture.md`, including replace-not-restore Node identity, external recovery-watermark, stale-restore, split-brain, and explicit database-security candidate rules; the disposition packet in `production-identity-storage-disposition-packet.md`; and the source review plus exact-candidate remediation re-review in `production-identity-storage-source-review.md`. The re-review verified all five medium findings fixed and made the lane eligible for a later post-RC architecture decision record, but did not authorize that record, `PIS-001`, a database dependency, schema/migration work, or runtime Postgres. The launch bundle, response kit, fail-closed closure gate, storage architecture decision, migration plan, backup/restore plan, retention model, and failure-mode tests remain required evidence. |
