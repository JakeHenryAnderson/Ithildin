...
| `ERG-006` | Production identity and multi-user authorization | `planning_only` | Enterprise identity, RBAC, tenant/team authorization, and remote admin use | The reviewed PIS-003 environment-evidence authority lineage now requires external operator target identity and signed receipt inputs before any separately reviewed collection-action authority can be prepared. Historical review lineage remains in `production-identity-storage-disposition-packet.md`, `production-identity-storage-external-review-bundle.md`, `production-identity-storage-response-kit.md`, `production-identity-storage-response-dry-run.md`, and `production-identity-storage-external-response-intake.md`. The wait state does not authorize target selection by the repository, receipt collection, dependency changes, production identity, enterprise RBAC, remote administration, or runtime work. The closed authority record and `production-identity-storage-disposition-closure-gate.md` remain governing evidence. |
| `ERG-007` | Durable runtime storage and retention | `planning_only` | Runtime Postgres, multi-user concurrency, retention policy, backup/restore, and production custody | The reviewed PIS-003 environment-evidence authority lineage now requires external operator target identity and signed receipt inputs before any separately reviewed collection-action authority can be prepared. The wait state does not authorize target selection by the repository, receipt collection, dependencies, schemas, migrations, runtime PostgreSQL, backup/restore behavior, retention enforcement, or production custody. The closed authority record, `production-identity-storage-disposition-closure-gate.md`, and database security profile remain governing evidence. |
