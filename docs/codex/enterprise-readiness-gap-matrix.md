...
| `ERG-002` | Mission Control display/importer | `planning_only` | Mission Control runtime importer and operator dashboard integration | Mission Control-side implementation plan, display-only schema, stale/mismatched packet fixtures, source-review handoff, and post-RC decision record; Ithildin-side importer planning exists in `mission-control-display-importer-plan.md` |
