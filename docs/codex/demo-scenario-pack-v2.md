...
| Operator workbench | `make workbench-readiness`, `make workbench-evidence-packet`, `make demo-workbench` | Read-only local operator workbench gate, focused packet, and evidence-only wrapper tying Agent Runs, approvals, audit status, live-demo artifacts, sandbox/workspace posture, and handoff pointers together. | Run controls, sandbox orchestration, SIEM adapters, production identity, or new governed tool powers. |
