...
- Bridge source identity: repo-owned `ithildin_node_mcp_bridge` from the same exact reviewed
  implementation candidate as the Node.
- Bridge entrypoint:
  `/opt/ithildin/.venv/bin/python -m ithildin_node_mcp_bridge`
...
- Build evidence must record the source commit, base OCI index and platform manifest, closed-profile
  SHA-256, dependency lock digest, final image ID/digest, platform, and SBOM/license receipt before a
  live journey.
...
The operator profile contains a fixed instruction that names only the three bridge affordances.
Gateway mission content never supplies a free-form prompt. The bridge may relay a bounded governed
tool result to the active runner process, but it does not persist the prompt, tool result, model
output, reasoning, Node private key, API admin token, provider credential, or local handoff nonce.
...
The configured model provider remains authoritative for inference facts.
