...
4. Run `make enterprise-review-send-quickstart`.
5. Run `make enterprise-review-submission-prompt`.
...
9. After a response arrives, run `make enterprise-dual-response-inbox`, open
   ...
   run `make enterprise-response-now`, run `make enterprise-response-paste-preflight`, and then run
   the lane-specific normalization, dry-run, and closure gate before any committed status update.
