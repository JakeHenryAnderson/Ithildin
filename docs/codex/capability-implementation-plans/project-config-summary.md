# Implementation-Planning Packet: project.config.summary
...
A future manifest may define `project.config.summary` as `risk: read`, category `project`, and
...
platform profiles, config value requests, package-manager requests, network requests, and roots
outside the selected workspace.

## Proposed Output Contract

Output remains count-only config posture metadata and allowlisted labels only:

- visible config directory and config-like file counts within configured depth and item limits;
- config-category counts, such as `build_config`, `test_config`, `lint_format_config`,
  `runtime_app_config`, `container_deployment_config`, `editor_tooling_config`,
  `ci_workflow_config`, and `unknown_config`;
- config-location counts, such as `root_level`, `config_directory`, `source_adjacent_config`,
  `ci_directory`, `tooling_directory`, and `unknown_location`;
...
It must include no config file names, no raw paths, no raw recursive listing, no stable
cross-response path identifiers, no file contents, no config contents, no config values, no
dependency names, no package names, no package script names or values, no environment names or
values, no registry URLs, no config parser output, no command output, no package-manager
stdout/stderr, no package-manager execution, no config parser execution, and no network access.

Boundary shorthand: no config file names; no config contents; no config values; no file contents;
no dependency names; no environment names or values; no package-manager execution; no network
access.
...
contents to infer config metadata, must not execute config parsers, and must not expose raw
filesystem errors.
...
- config categories: `build_config`, `test_config`, `lint_format_config`, `runtime_app_config`,
  `container_deployment_config`, `editor_tooling_config`, `ci_workflow_config`, and
  `unknown_config`;
- config locations: `root_level`, `config_directory`, `source_adjacent_config`, `ci_directory`,
  `tooling_directory`, and `unknown_location`;
- skipped count categories: hidden/sensitive paths, `.git`, symlinks, hardlinks, unsupported
  entries, depth limits, item limits, and safe errors.

Unknown categories must aggregate as `unknown_config` or `unknown_location`; they must not create
caller- or repository-derived output keys that reveal raw names, raw paths, or raw extensions.
...
resource construction for resource type `project_config`.

## Audit Evidence Plan

Audit fields should record only safe metadata: tool name, workspace ID, resource type, root label,
max depth, limit, config-category keys, config-location keys, skipped count keys, truncation state,
output policy booleans, policy hash, manifest hash, schema hash, request hash, and principal ID. No
raw paths, file names, file contents, config contents, config values, dependency names, package
names, scripts, environment names or values, command output, registry URLs, or raw filesystem
errors may be audited.

## UI And Policy Preview Plan

Review-console display may show compact count summaries and skipped/truncated warnings only. It
must not add mutation controls, config parser controls, recursive browsing, file-content preview,
source-code search, package lookup, dependency browsing, deployment-readiness claims, compliance
conclusions, sandbox controls, or SIEM controls.
...
regex, oversized/deep directory tree, raw file-name request, file-content request, config-content
request, config-value request, package script request, package-manager execution request, config
parser execution request, environment-variable request, registry/network request, and unknown
principal.
...
implementation decision is recorded for this one bounded read-only config-summary capability.
