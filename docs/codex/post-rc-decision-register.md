...
| `PRD-COMPLIANCE-MAPPING-001` | Compliance mapping support lane | `approved_for_planning` | Mapping-template architecture, operator responsibility language, legal-review boundary, and evidence-field planning only | `false` | The architecture packet in `compliance-mapping-architecture.md`, the synthetic template corpus in `compliance-mapping-template-compatibility-fixtures.md`, the disposition packet in `compliance-mapping-disposition-packet.md`, the response intake template in `compliance-mapping-external-response-intake.md`, the fail-closed closure gate in `compliance-mapping-disposition-closure-gate.md`, the response dry-run fixture checker in `compliance-mapping-response-dry-run.md`, exact real-framework scope and version, accepted-risk impact review, post-RC decision record, and external/source review before runtime work |
