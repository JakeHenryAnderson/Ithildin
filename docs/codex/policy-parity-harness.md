...
Task 096 adds a repeatable local harness for checking that policy preview decisions match the
policy decision evidence emitted by the governed runtime path.
...
write approval requirement, write-proposal allow, allowlisted network allow, and out-of-scope
network denial.
