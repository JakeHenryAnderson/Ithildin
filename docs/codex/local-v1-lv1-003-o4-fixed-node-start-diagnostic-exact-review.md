...
- `M1`: a successful fixed-Node start return bypassed the existing forbidden-output validation
  before Hermes could run.
- `L1`: a post-bind or pre-discard anchor-validation failure could leave the diagnostic container
  identity transiently bound after the diagnostic should have relinquished it.
